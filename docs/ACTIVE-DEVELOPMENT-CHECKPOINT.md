...
- PR #396 merged as `f65b748543d44f420bd8e9b01e6b511a3c46c19c`, adding guarded Terraform provisioning for the free-first OCI host. The module pins Terraform `1.16.4` and OCI provider `8.29.0`, hard-codes `VM.Standard.A1.Flex` at 2 OCPUs / 12 GiB RAM with a 50 GB boot volume, creates only the required VCN/public subnet/internet gateway/minimal security list/NSG rules plus one public instance, and has no paid-shape fallback.
- The OCI Terraform plan guard rejects unknown resource-create types, non-A1 shapes, CPU/memory drift beyond the guarded Always Free profile, oversized boot volume, private-only networking and multiple instance creates. PR and merged-main OCI Terraform validation passed, including provider initialization, `terraform validate`, plan-guard unit tests and free-profile safety locks.
- PR #398 merged as `6269a76a71432afa0ab8d8cb5c64befd0714c11e`, adding the browser-only OCI Cloud Shell activation path. The helper defaults to plan-only, discovers the signed-in tenancy home region from the pre-authenticated OCI session, generates a Cloud-Shell-only Ed25519 SSH key when needed, requires a restricted `ADMIN_SSH_CIDR`, verifies the guarded Terraform plan, and requires a separate explicit `--apply` invocation. It contains no `-auto-approve` path and no paid-resource fallback.
- Merged-main OCI Terraform Free Guard, OCI ARM64 Telecom Lock, Telecom Core Verification, Owned SIP/PSTN Core Lock, Full Platform QA, real Chromium Native WebRTC Browser Media E2E, Railway exact-commit classification, Build and Deploy classification, Verify Magnanimous Telecom Production and Voice Conversation Production Smoke all passed for the Cloud Shell path. The production classifiers correctly made **no Worker or Railway runtime deployment** for this control/infrastructure-only change.
- The real Chromium proof after PR #398 again registered an ephemeral SIP browser identity, established the Echo call, proved bidirectional browser media, revoked the temporary credential and verified Asterisk removed the ephemeral endpoint.
- Oracle's current official Always Free documentation confirms A1 is 2 OCPUs + 12 GB memory total for Always Free tenancies. Cloud Shell is free, browser-based and pre-authenticated; Oracle's Terraform guidance says Cloud Shell/Resource Manager do not require separate RSA API signing keys.

Exact next external action:
- create/sign in to an Oracle Cloud tenancy. Oracle account creation and any required phone/card verification are the only remaining unavoidable user-only steps before the free-first host can be provisioned;
- once signed in, open OCI Cloud Shell and use `telecom-core/oci/cloud-shell-plan.sh` with a restricted `ADMIN_SSH_CIDR`. It performs a guarded **plan only** by default and creates nothing;
- after reviewing the verified free-profile plan, rerun the helper explicitly with `--apply` to create only the guarded `VM.Standard.A1.Flex` 2-OCPU / 12-GiB / 50-GB public host path. If A1 capacity is unavailable, stop or try another availability domain; do not fall back to a paid shape;
- then point Telecom DNS to the Terraform `public_ip`, run `telecom-core/deploy/oci-always-free-preflight.sh`, configure trusted TLS + protected Telecom Core secrets, start the already-verified Telecom Core and run `Public Telecom WebRTC Verification`;
