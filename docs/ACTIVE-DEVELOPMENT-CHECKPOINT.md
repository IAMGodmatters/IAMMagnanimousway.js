...
- [x] Implement the private Stasis-managed source lifecycle: authenticated ARI event WebSocket, Stasis-owned channel registry, managed mixing bridges, separate snoop/supervisor bridges for monitor/whisper/barge, consent-gated bridge recording, stop/store handling, and orphan cleanup. Source/unit verification is required before merge and the runtime gate remains disabled by default.
- [x] Persist/recover managed Stasis bridge, supervisor-session, recording and consent/audit state in the owned Telecom PostgreSQL plane. Startup restores active state, ARI reconnect reseeds current Stasis application channels, and missing Asterisk objects are reconciled back to ended durable audit state. Production activation still requires deployed restart/recovery proof.
- [ ] Enable `ASTERISK_STASIS_ENABLED` only on the dedicated Telecom Core, prove a real carrier-free channel enters Stasis, prove a managed two-channel bridge plus monitor/whisper/barge media and consent-gated bridge recording end-to-end, then add the owner/admin platform handoff. Do not expose or label supervisor audio/recording live before that deployed proof.
