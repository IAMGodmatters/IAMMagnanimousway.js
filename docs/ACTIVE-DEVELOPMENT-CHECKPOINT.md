...
- PR #396 merged as `f65b748543d44f420bd8e9b01e6b511a3c46c19c`, adding guarded Terraform provisioning under `telecom-core/oci/`. The module pins Terraform 1.16.4 and OCI provider 8.29.0, hard-codes `VM.Standard.A1.Flex` at 2 OCPUs / 12 GiB RAM with a 50 GB boot volume, creates only the required VCN/public subnet/internet gateway/minimal security list/NSG plus one public A1 instance, restricts SSH to an explicit admin CIDR, and has no paid-shape fallback.
- The Terraform plan guard rejects unexpected resource-create types, non-A1 shape drift, CPU/memory drift, oversized boot volume, private-only networking, and multiple instance creates. Merged-main OCI Terraform Free Guard run `35989471740` passed unit tests, formatting, provider initialization, `terraform validate`, and the static free-profile contracts.
- Merged-main OCI ARM64 Telecom Lock `35989471726` passed after #396: Asterisk 22.11.0, Telecom Core API, Kamailio, coturn and PostgreSQL all built/verified on native ARM64 and the guarded Compose model validated. Merged-main Native WebRTC Browser Media E2E `35989471901` also passed, preserving real Chromium registration, authenticated calling, two-way media, revocation and endpoint cleanup.
- Merged-main Full Platform QA `35989471553` passed. Build and Deploy I AM `35989471598` correctly classified #396 as no production-runtime change and skipped the deploy job; Railway exact-commit run `35989471719` likewise passed without changing the existing Railway service. Voice Conversation Production Smoke `35989491924` correctly recognized that skipped deploy and completed successfully instead of reporting a false voice failure.
- OCI Cloud Shell is the preferred execution surface after tenancy creation because Oracle Cloud Shell is already authenticated for OCI tooling; do not create or commit API private keys merely to run this module when Cloud Shell is available.

Exact next external action:
- create/sign in to an Oracle Cloud tenancy; account creation and Oracle's identity/phone/card verification, when required, are the remaining user-only authorization steps;
- from OCI Cloud Shell, use the guarded `telecom-core/oci/` module to plan exactly one **Always Free `VM.Standard.A1.Flex`** host at 2 OCPUs / 12 GiB RAM / 50 GB boot with a public IPv4. Run `verify_free_plan.py` before apply and stop rather than selecting a paid shape if Always Free capacity is unavailable;
- point the chosen Telecom hostname to the resulting public IPv4, run `telecom-core/deploy/oci-always-free-preflight.sh`, configure trusted TLS and protected Telecom Core secrets, and deploy the already-verified Telecom Core;
- run `Public Telecom WebRTC Verification` from the external GitHub-hosted runner; only after trusted TLS/WSS plus real bidirectional media and remote audio are green may `TELECOM_NATIVE_WEBRTC_LIVE=true` be promoted.
