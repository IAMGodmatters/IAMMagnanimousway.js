...
- create/sign in to an Oracle Cloud tenancy and provision an **Always Free `VM.Standard.A1.Flex`** instance in the tenancy home region with 2 OCPUs, 8-12 GiB RAM, Ubuntu, a public subnet, and a public IPv4; do not upgrade the account or choose a paid shape merely to obtain capacity;
- then run `telecom-core/deploy/oci-always-free-preflight.sh`, configure DNS + trusted TLS, deploy the already-verified Telecom Core, and run `Public Telecom WebRTC Verification`;
