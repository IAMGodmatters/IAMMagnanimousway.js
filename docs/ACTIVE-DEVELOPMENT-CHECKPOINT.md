...
Status: **Deep telecom/carrier/contact-center software architecture is merged; this branch adds an explicit fail-closed generic BYOC planner contract on top of the already-merged Magnanimous Telecom Core plus Twilio/Plivo selected-route execution. Consent-gated Stasis supervision/recording and real-host verification harness are merged; native Chromium↔Asterisk media is CI-verified. Public-host activation, real-host Stasis evidence, regulatory authority and final retirement of no-route legacy fallback stay separately gated.**
...
- [x] Migrate generic workspace BYOC into explicit planner execution without leaking protected Telecom Core routing: `byoc-bridge` routes may opt in with an opaque `policy.byoc_route_key`; the owner must separately enable `VOIP_BYOC_ROUTE_CONTRACT=magnanimous-route-v1`; the bridge receives only sanitized route attribution/quality data plus that opaque route key; unbound selected BYOC routes fail closed.
- [ ] Retire the no-route legacy Plivo/Twilio compatibility fallback only after affected tenants have explicit route configuration. Until then `live_execution_uses_route_planner` remains false even though protected Telecom Core, configured generic BYOC, Twilio and Plivo selected routes are planner-controlled.
