...
- [x] Add the generic BYOC selected-route contract: disabled by default, separate non-secret `bridge_route_key`, same-origin HTTPS authenticated health preflight, explicit owner enablement, and exact route/interconnect/key confirmation before Magnanimous records the route as applied.
- [ ] Retire no-route legacy generic/Twilio/Plivo compatibility fallback only after affected tenants have explicit route configuration. Until then `live_execution_uses_route_planner` remains false even though configured native, generic BYOC, Twilio and Plivo routes have guarded planner-controlled execution paths.
