...
- [x] Research and durably absorb the Fonus/Popcorn global-mobile retail patterns into an original Magnanimous architecture: provider-neutral SIM/eSIM access, backup profile/network steering, app voice/text fallback, multiple-number identity, fair-use/high-speed policy, truthful 2FA limits, a separate `TELECOM_GLOBAL_MOBILE_LIVE=false` gate, and verified-origin 20% retail quote controls with funded variable-cost caps. The benchmark is versioned in `docs/GLOBAL-MOBILE-RETAIL-BENCHMARK-2026-09-26.md`; no provider contract or live global-mobile claim is implied.\n- [x] Tighten Asterisk recording lifecycle truth: after live recording stop or auto-completion, verify `GET /recordings/stored/{name}` before reporting stored state; keep raw recording files private.\n- [ ] Obtain required NTC/FCC/other authorizations, carrier agreements, numbering/emergency-service arrangements before representing those regulated capabilities as live.
