...
- A free-first OCI host path is now being prepared additively rather than creating a paid VPS: Oracle's current Always Free A1 shape can provide the public Linux host class required by the direct RTP architecture, while repository preflight and native ARM64 CI are required before treating the exact Magnanimous Telecom stack as OCI-source-ready. Oracle account creation/phone/card verification remains an unavoidable user-only external action and no OCI resource has been created.
