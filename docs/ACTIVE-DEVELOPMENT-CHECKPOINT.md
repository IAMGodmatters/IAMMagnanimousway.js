...
Status: **Deep telecom/carrier/contact-center software architecture is merged; protected selected-route execution covers Magnanimous Telecom Core plus explicit Twilio/Plivo routes; generic BYOC now has an opt-in same-origin selected-route contract while legacy BYOC/no-route fallbacks remain available; consent-gated Stasis supervision/recording source and its guarded real-host evidence harness are merged; native Chromium↔Asterisk media is CI-verified. Public-host activation, real-host Stasis evidence, regulatory authority and legacy-fallback retirement stay separately gated.**
...
- [x] Add an opt-in generic BYOC selected-route contract. A `byoc-bridge` route can declare an opaque `policy.bridge_route_key`; Magnanimous probes a same-origin HTTPS `magnanimous.carrier-route.v1` capability endpoint, sends only the opaque key, rejects redirects/cross-origin probes, and requires the bridge to confirm the exact route key before reporting the route applied.
- [ ] Migrate affected tenants away from non-opted-in generic BYOC and no-route legacy Plivo/Twilio fallbacks, then retire those fallback paths. Until that migration is evidenced, `live_execution_uses_route_planner` remains false even though protected Telecom Core, explicit Twilio/Plivo routes, and contract-capable generic BYOC are planner-controlled.
