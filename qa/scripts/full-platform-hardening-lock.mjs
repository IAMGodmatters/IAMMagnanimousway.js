...
mustContain(authority,'export async function authenticatedSessionUser','Central session authority must expose one user resolver for opaque and signed compatibility sessions.');
mustContain(authority,"if(isOpaqueToken(token))",'Central user resolution must accept opaque sessions directly.');
mustContain(authority,'const parts=await verifiedLegacyParts(token,env)','Central user resolution must preserve the signed-session compatibility rail.');
mustContain(integrations,"from './session-authority.js'",'Shared integration authentication must use the central session authority.');
mustContain(integrations,'return authenticatedSessionUser(request,env);','Shared currentUser authentication must accept opaque sessions directly instead of parsing only legacy bearer tokens.');
