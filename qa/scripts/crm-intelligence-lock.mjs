...
has(runtime,'crm_accounts','CRM stores first-class company/account records');
has(runtime,'crm_pipelines','CRM stores multiple sales pipelines');
has(runtime,'crm_pipeline_stages','CRM stores configurable pipeline stages');
has(runtime,"/api/operations/crm/studio",'CRM studio endpoint exists');
has(runtime,"/api/operations/crm/accounts",'company/account API exists');
has(runtime,"/api/operations/crm/pipelines",'configurable pipeline API exists');
has(runtime,'crm_contact_account_updated','contact-to-account relationships are audited');
has(runtime,'crm_pipeline_created','pipeline creation is audited');
has(ui,'COMPANY / ACCOUNT GRAPH','CRM UI exposes company/account rollups');
has(ui,'SALES PROCESS STUDIO','CRM UI exposes configurable pipeline studio');
has(ui,'+ PIPELINE','CRM UI exposes pipeline creation');
has(ui,'+ ACCOUNT','CRM UI exposes account creation');
