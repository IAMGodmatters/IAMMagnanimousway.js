...
add('credential vault rewrap remains on the Cloudflare source vault instead of the standalone target',securityEntry.includes("url.pathname==='/api/internal/migration/rewrap-platform-credentials'")&&securityEntry.indexOf("url.pathname==='/api/internal/migration/rewrap-platform-credentials'")<securityEntry.indexOf('const origin=configuredStandaloneApiOrigin(env);'));
add('private edge AI bridge is excluded from the standalone proxy',securityEntry.includes("url.pathname==='/api/internal/edge-ai/run'")&&securityEntry.indexOf("url.pathname==='/api/internal/edge-ai/run'")<securityEntry.indexOf('const origin=configuredStandaloneApiOrigin(env);'));
add('private edge AI bridge executes on Worker before API proxying',securityEntry.includes('const edgeAiResponse=await handleEdgeAiBridge(request,env);')&&securityEntry.includes('const standaloneApiResponse=await proxyApiToStandalone(request,env);')&&securityEntry.indexOf('const edgeAiResponse=await handleEdgeAiBridge(request,env);')<securityEntry.indexOf('const standaloneApiResponse=await proxyApiToStandalone(request,env);'));
...
add('deploy derives and syncs a private edge AI bridge token without exposing its value',deploy.includes('magnanimous-edge-ai-bridge-v1')&&deploy.includes('MAGNANIMOUS_EDGE_AI_BRIDGE_TOKEN')&&deploy.includes('without exposing its value'));
...
add('Stripe bootstrap read verification defers only after D1 was already quota-deferred',deploy.includes('if [ "${D1_MIGRATION_DEFERRED:-0}" = "1" ]')&&deploy.includes('Preserving the previously verified encrypted Stripe bootstrap state'));
add('payment/bootstrap-sensitive commits fail closed when D1 quota blocks Stripe verification',deploy.includes('payment_bootstrap_sensitive=1')&&deploy.includes('Stripe webhook bootstrap verification cannot be deferred safely.'));
add('Stripe bootstrap quota defer does not classify telecom-only Worker files as payment sensitive',deploy.includes("grep -Eq '^(worker/migrations/0084_|worker/src/[^/]*(stripe|billing|payment|checkout|pricing|premium)[^/]*\\.js$|frontend/app/(billing|pricing|checkout)/|api/(stripe|billing|payment)/)'"));
add('deferred Stripe bootstrap verification is recorded explicitly for later reconciliation',deploy.includes('STRIPE_BOOTSTRAP_CHECK_DEFERRED=1'));
add('deploy checkout includes parent commit for migration diff safety',/fetch-depth:\s*(?:[2-9]|[1-9]\d+)\b/.test(deploy)&&deploy.includes('git -C .. diff --quiet HEAD^ HEAD -- worker/migrations'));
