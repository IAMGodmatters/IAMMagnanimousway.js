...
add('credential vault rewrap and internal AI bridge remain on the Cloudflare source runtime instead of the standalone target',securityEntry.includes("url.pathname==='/api/internal/migration/rewrap-platform-credentials'")&&securityEntry.includes("url.pathname==='/api/internal/ai/run'")&&securityEntry.indexOf("url.pathname==='/api/internal/migration/rewrap-platform-credentials'")<securityEntry.indexOf('const origin=configuredStandaloneApiOrigin(env);')&&securityEntry.indexOf("url.pathname==='/api/internal/ai/run'")<securityEntry.indexOf('const origin=configuredStandaloneApiOrigin(env);'));
