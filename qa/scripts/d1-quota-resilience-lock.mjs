...
add('Worker publish and production smoke precede optional full-brain cache sync',deploy.indexOf('- name: Deploy Worker')<deploy.indexOf('- name: Production smoke test')&&deploy.indexOf('- name: Production smoke test')<deploy.indexOf('- name: Materialize full Magnanimous capability brain'));
add('optional full-brain cache sync cannot fail or indefinitely block a valid release',deploy.includes('- name: Materialize full Magnanimous capability brain\n        continue-on-error: true\n        timeout-minutes: 12'));
