...
const adminMigration=read('worker/migrations/0081_admin_auth_runtime_schema.sql');
...
const adminEnsureStart=adminCompat.indexOf('async function ensureTables');
const adminEnsureEnd=adminCompat.indexOf('async function ensureLegacyCompatibility',adminEnsureStart);
const adminEnsure=adminCompat.slice(adminEnsureStart,adminEnsureEnd);
const adminLegacyStart=adminCompat.indexOf('async function ensureLegacyCompatibility');
const adminLegacyEnd=adminCompat.indexOf('function d1DailyLimit',adminLegacyStart);
const adminLegacy=adminCompat.slice(adminLegacyStart,adminLegacyEnd);
add('admin account schema is migration-owned',adminEnsure.includes('SELECT 1 FROM auth_events LIMIT 1')&&!adminEnsure.includes('CREATE TABLE')&&!adminEnsure.includes('CREATE INDEX'));
add('admin legacy compatibility no longer alters schema at request time',!adminLegacy.includes('ALTER TABLE'));
add('admin account layer preserves D1 write quota as explicit 503',adminCompat.includes("D1_DAILY_ROW_WRITE_LIMIT")&&adminCompat.includes('},503)'));
add('admin account layer preserves D1 read quota as explicit 503',adminCompat.includes("D1_DAILY_ROW_READ_LIMIT"));
add('admin auth migration owns auth_events',adminMigration.includes('CREATE TABLE IF NOT EXISTS auth_events'));
add('admin auth migration owns auth_config fallback',adminMigration.includes('CREATE TABLE IF NOT EXISTS auth_config'));
...
add('migration defer proves all required 0080 tables already exist',deploy.includes("'tenant_settings','ads','settings','security_rate_limits','auth_events'")&&deploy.includes('required production tables already exist'));
