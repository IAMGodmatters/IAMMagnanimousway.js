...
const platformSource=fs.readFileSync(new URL('../../worker/src/platform-credentials.js',import.meta.url),'utf8');
const providerSource=fs.readFileSync(new URL('../../worker/src/provider-runtime-env.js',import.meta.url),'utf8');
assert.match(platformSource,/runtimeEnvOverlay\(env\)/);
assert.doesNotMatch(platformSource,/const merged=\{\.\.\.env\}/);
