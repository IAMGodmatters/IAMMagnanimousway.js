...
includes(standalonePage, "data-iam-speak={m.speak===false?'off':'on'}", 'voice reliability: standalone marks technical failures as non-spoken messages');
includes(voiceOrchestrator, "latest.dataset.iamSpeak==='off'", 'voice reliability: orchestrator refuses to speak messages marked non-spoken');
includes(voiceOrchestrator, 'maxChunkChars:iosSpeech?145:220', 'voice reliability: iOS WebKit uses shorter speech chunks');
includes(voiceOrchestrator, 'interChunkDelayMs:iosSpeech?125:60', 'voice reliability: iOS WebKit receives a longer inter-chunk settle delay');
includes(naturalSpeech, 'export function isIosSpeechRuntime()', 'voice reliability: iOS speech runtime detection is centralized');
includes(naturalSpeech, 'if((synth as any).paused)synth.resume?.()', 'voice reliability: speech synthesis is resumed only when actually paused');
includes(naturalSpeech, 'if(ios)window.setTimeout(next,70);else next();', 'voice reliability: iOS starts speech after the cancel queue settles');
