...
import { MAGNANIMOUS_BUILDER_TOOL_CONTRACTS, getMagnanimousBuilderToolPolicy } from '../../worker/src/magnanimous-builder-capability-catalog.js';
import { MAGNANIMOUS_ENGINEERING_GUIDE_TOPICS, MAGNANIMOUS_ENGINEERING_TECHNIQUE_PROFILES, getMagnanimousTechniqueProfile, getMagnanimousTechniqueSummary } from '../../worker/src/magnanimous-engineering-technique-catalog.js';
...
const builderResearch=read('worker/src/magnanimous-builder-capability-catalog.js');
const engineeringTechniques=read('worker/src/magnanimous-engineering-technique-catalog.js');
...
const downstreamDispatchIndex=Math.max(security.indexOf('const rawResponse = await app.fetch(policyRequest, env, ctx);'),security.indexOf('const rawResponse = await app.fetch(policyRequest, executionEnv, ctx);'));
checks.push(['Tool Foundry routing occurs before downstream app dispatch',security.indexOf('const toolFoundryResponse=await handleMagnanimousToolFoundry(policyRequest,env);')>=0&&downstreamDispatchIndex>=0&&security.indexOf('const toolFoundryResponse=await handleMagnanimousToolFoundry(policyRequest,env);')<downstreamDispatchIndex]);
lacks(security,"if(url.pathname.startsWith('/api/magnanimous/tool-foundry'))return null;",'Tool Foundry remains on the shared standalone session plane');
has(deployWorkflow,'Wait for standalone session plane revision','production deploy waits for Railway session-plane parity');
has(deployWorkflow,'Shared session plane ready for smoke','deployment records session-plane readiness before smoke');
checks.push(['session-plane parity gate precedes production smoke',deployWorkflow.indexOf('Wait for standalone session plane revision')>=0&&deployWorkflow.indexOf('Wait for standalone session plane revision')<deployWorkflow.indexOf('Production smoke test')]);
...
has(engineeringTechniques,'MAGNANIMOUS_ENGINEERING_TECHNIQUE_PROFILES','Magnanimous has a first-party engineering technique catalog');
has(engineeringTechniques,"mode:'magnanimous-first-party-technique-catalog'",'engineering techniques are explicitly first-party Magnanimous');
...
const runtimeAbsorption=liveAbsorptionCatalog(),runtimeManifest=liveCapabilityManifest(),persistentManifest=getPersistentConnectorAbsorptionManifest(),pluginManifest=getChatGPTPluginCapabilityManifest(),skillManifest=getInstalledPluginSkillManifest(),builderManifest=getMagnanimousBuilderCapabilityManifest(),engineeringManifest=getMagnanimousEngineeringSkillManifest(),runtimeSummary=getConnectorAbsorptionSummary(),liveToolSummary=getLivePluginToolResearchSummary(),liveSkillSummary=getLivePluginSkillResearchSummary(),realizations=runtimeManifest.map(classifyCapabilityRealization);
...
checks.push(['all 65 Magnanimous engineering topics have explicit technique profiles',Object.keys(MAGNANIMOUS_ENGINEERING_TECHNIQUE_PROFILES).length===65&&engineeringTechniqueSummary.guide_profiles===65]);
checks.push(['every Magnanimous engineering topic resolves to a non-empty technique profile',MAGNANIMOUS_ENGINEERING_GUIDE_TOPICS.every(x=>getMagnanimousTechniqueProfile(x.id).techniques.length>0)]);
checks.push(['every Magnanimous engineering skill carries reusable technique metadata',engineeringManifest.every(x=>Array.isArray(x.techniques)&&x.techniques.length>0&&x.research?.techniques?.length>0)]);
checks.push(['every Magnanimous engineering skill carries safe suggestive initiative metadata',engineeringManifest.every(x=>x.initiative?.suggestive===true&&x.initiative?.auto_initiate===true&&x.initiative?.action_class==='skill-guidance')]);
checks.push(['Magnanimous Builder list/read inspection can auto-initiate safely',getMagnanimousBuilderToolPolicy('list_projects').auto_initiate===true&&getMagnanimousBuilderToolPolicy('read_file').auto_initiate===true]);
checks.push(['Magnanimous Builder SQL mutation and publishing remain confirmation-gated',getMagnanimousBuilderToolPolicy('execute_sql').requires_confirmation===true&&getMagnanimousBuilderToolPolicy('publish_app').requires_confirmation===true]);
checks.push(['every Magnanimous Builder capability carries suggestive initiative metadata',builderManifest.every(x=>x.initiative?.suggestive===true&&typeof x.initiative?.auto_initiate==='boolean'&&typeof x.initiative?.requires_confirmation==='boolean')]);
...
checks.push(['Magnanimous Builder summary is exposed through the absorption overview',runtimeSummary.magnanimous_builder_tools===44&&runtimeSummary.builder?.builder_tools===44&&runtimeSummary.first_party_builder_runtime===true]);
checks.push(['Magnanimous Engineering summary is exposed through the absorption overview',runtimeSummary.magnanimous_engineering_skills===65&&runtimeSummary.magnanimous_engineering_technique_profiles===65&&runtimeSummary.magnanimous_reusable_engineering_techniques===engineeringTechniqueSummary.reusable_techniques&&runtimeSummary.magnanimous_reusable_engineering_techniques>65]);
