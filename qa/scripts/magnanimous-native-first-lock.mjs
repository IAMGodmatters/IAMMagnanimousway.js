...
has(security,"if(url.pathname.startsWith('/api/magnanimous/tool-foundry'))return null;",'standalone proxy cannot intercept Tool Foundry before auth resolution');
checks.push(['Tool Foundry proxy bypass lives inside standalone proxy gate',security.indexOf("if(url.pathname.startsWith('/api/magnanimous/tool-foundry'))return null;")>security.indexOf('async function proxyApiToStandalone')&&security.indexOf("if(url.pathname.startsWith('/api/magnanimous/tool-foundry'))return null;")<security.indexOf('const origin=configuredStandaloneApiOrigin(env);')]);
