...
has(carrier,"byoc_route_key must be an opaque workspace bridge route key, not a URL or credential.",'generic BYOC route key rejects URLs and credentials');
has(carrier,"Only a byoc-bridge interconnect can declare byoc_route_key.",'generic BYOC planner binding is limited to BYOC interconnects');
has(carrier,"Choose either asterisk_endpoint or byoc_route_key for one route, not both.",'one route cannot mix protected Core and generic bridge execution contracts');
has(carrier,'bridge_route_key:bridgeRouteKey','planner carries a separate opaque generic BYOC selector');
has(carrier,"bridgeRouteKey?'workspace-byoc':'preview-only'",'route creation reports planner-bound workspace BYOC separately from preview-only routes');
...
has(leadPhone,"provider.origin === core.origin",'protected Asterisk endpoint payload remains same-origin with Telecom Core');
has(leadPhone,"VOIP_BYOC_ROUTE_CONTRACT || '').trim() === 'magnanimous-route-v1'",'generic BYOC planner execution requires an explicit owner bridge contract');
has(leadPhone,"route_key: String(selected.bridge_route_key).trim()",'generic BYOC receives only the opaque planner route key');
has(leadPhone,"execution_adapter: 'workspace-byoc'",'generic BYOC route payload is explicitly scoped to the workspace bridge adapter');
lacks(leadPhone,'route_key: String(selected.endpoint','legacy interconnect endpoint cannot leak into generic BYOC route selection');
has(phoneCarrier,"code: 'SELECTED_BYOC_ROUTE_NOT_BOUND'",'planner-selected BYOC fails closed when no explicit execution contract is bound');
has(phoneCarrier,'selectedBridgeRouteKey && workspaceByocPlannerReady(env)','compatibility router delegates generic BYOC only when both route key and owner contract are present');
...
has(network,'selected_route_execution:{native_telecom_core:true,generic_byoc:true,twilio_compatibility:true,plivo_compatibility:true}','network truth reports explicit planner-bound generic BYOC alongside protected Core and compatibility adapters');
has(network,'No-route legacy compatibility fallback remains outside full planner authority','network truth keeps all-live-route authority false until legacy no-route fallback is retired');
