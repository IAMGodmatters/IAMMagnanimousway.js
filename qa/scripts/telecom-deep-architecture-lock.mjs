...
has(telecomRequirements,'websockets==17.1','private ARI event listener pins the reviewed current WebSocket client');
has(telecomConfig,'stasis_enabled: bool = False','Stasis call control remains disabled by default');
has(telecomConfig,'ASTERISK_STASIS_ENABLED','Stasis activation requires an explicit protected runtime setting');
has(compose,'ASTERISK_STASIS_ENABLED: ${ASTERISK_STASIS_ENABLED:-false}','Compose preserves the disabled-by-default Stasis gate');
...
has(stasisControl,'/snoop/','supervisor audio uses native Asterisk snoop channels');
has(stasisControl,'"monitor": ("both", "none")','monitor mode spies without whisper injection');
has(stasisControl,'"whisper": ("both", "out")','whisper mode injects only the configured target-side direction');
has(stasisControl,'"barge": ("both", "both")','barge mode uses bidirectional whisper injection');
has(stasisControl,'Recording requires explicit consent confirmation and a consent basis.','bridge recording fails closed without affirmative consent evidence');
...
has(lifecycle,'container.stasis_events.run()','Telecom lifecycle owns the ARI event task');
