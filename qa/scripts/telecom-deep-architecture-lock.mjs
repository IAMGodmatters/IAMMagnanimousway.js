...
has(leadPhone,'genericByocRouteContractReady','outbound calls require the generic BYOC contract gate before sending route controls');
has(leadPhone,'selectedRouteConfirmed','outbound calls require exact bridge confirmation before claiming route application');
has(byocContract,"provider.origin === health.origin",'generic BYOC route health endpoint must share the carrier-call origin');
has(byocContract,"provider.protocol === 'https:'",'generic BYOC route contract requires HTTPS');
has(byocContract,"selected_route_contract !== 'magnanimous-selected-route-v1'",'generic BYOC route contract rejects unrecognized bridge capability contracts');
has(byocContract,"error.code = 'CARRIER_ROUTE_UNAVAILABLE'",'unhealthy or unverifiable generic BYOC routes fail closed');
has(byocContract,"String(provider.bridge_route_key || '') === String(selectedRoute.bridge_route_key || '')",'generic BYOC route confirmation must match the exact route key');
has(providerEnv,"'VOIP_PROVIDER_ROUTE_CONTROL_ENABLED'",'generic BYOC route-control enablement stays server-side');
has(providerEnv,"'VOIP_PROVIDER_HEALTH_URL'",'generic BYOC route health endpoint stays server-side');
has(platformCredentials,'Enable Generic BYOC Selected-Route Contract','owner Provider Vault exposes the route-control gate');
has(carrier,"bridge_route_key must be a stable non-secret route key.",'route creation validates generic bridge route keys');
has(carrier,"A route cannot use both asterisk_endpoint and bridge_route_key.",'route policy cannot ambiguously target native and generic execution simultaneously');
...
has(network,'selected_route_execution:{native_telecom_core:true,generic_byoc_contract_gated:Boolean','network truth reports generic BYOC route control only as owner-enabled contract-gated capability');
has(network,'twilio_compatibility:false,plivo_compatibility:false','network truth keeps Twilio/Plivo compatibility routes unmigrated');
