...
const byocContract=read('worker/src/byoc-route-contract.js');
...
has(leadPhone,'const routeApplied = selectedRouteConfirmed(provider, selectedRoute);','generic bridge route application is accepted only through exact contract confirmation');
...
has(leadPhone,'genericByocRouteContractReady','generic BYOC route controls require the explicit server-side contract gate');
has(leadPhone,'verifyGenericByocRoute','generic BYOC selected routes require authenticated health before dialing');
has(byocContract,"provider.origin === health.origin",'generic BYOC health endpoint must share the call endpoint origin');
has(byocContract,"provider.protocol === 'https:'",'generic BYOC selected-route contract requires HTTPS');
has(byocContract,"selected_route_contract !== 'magnanimous-selected-route-v1'",'generic BYOC bridge must advertise the expected contract version');
has(byocContract,"error.code = 'CARRIER_ROUTE_UNAVAILABLE'",'unhealthy or unverifiable generic BYOC routes fail closed');
has(byocContract,"String(provider.bridge_route_key || '') === String(selectedRoute.bridge_route_key || '')",'generic BYOC route confirmation must match the exact bridge route key');
has(providerEnv,"'VOIP_PROVIDER_ROUTE_CONTROL_ENABLED'",'generic BYOC route-control enablement remains server-side');
has(providerEnv,"'VOIP_PROVIDER_HEALTH_URL'",'generic BYOC health endpoint remains server-side');
has(platformCredentials,'Enable Generic BYOC Selected-Route Contract','owner Provider Vault exposes the generic route-control gate');
has(carrier,"bridge_route_key must be a stable non-secret route key.",'route creation validates generic bridge route keys');
has(carrier,"A route cannot use both asterisk_endpoint and bridge_route_key.",'route policy cannot target native and generic execution simultaneously');
...
has(network,'selected_route_execution:{native_telecom_core:true,generic_byoc_contract_gated:Boolean','network truth reports generic BYOC only as owner-enabled contract-gated routing');
has(network,'twilio_compatibility:true,plivo_compatibility:true','network truth preserves explicit Twilio/Plivo planner execution');
...
has(telecomWorkflow,'node --check worker/src/byoc-route-contract.js','Telecom CI syntax-checks the generic BYOC contract module');
has(telecomWorkflow,'node qa/scripts/byoc-route-contract-lock.mjs','Telecom CI executes the generic BYOC contract regression');
