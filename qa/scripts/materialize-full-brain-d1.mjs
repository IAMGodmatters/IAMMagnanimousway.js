...
ON CONFLICT(connector_id,capability_id) DO UPDATE SET connector_name=excluded.connector_name,category=excluded.category,native_target=excluded.native_target,boundary=excluded.boundary,source_kind=excluded.source_kind,status='tool-foundry-specified',research_json=excluded.research_json,spec_json=excluded.spec_json,updated_at=excluded.updated_at WHERE magnanimous_connector_capability_absorption.connector_name<>excluded.connector_name OR magnanimous_connector_capability_absorption.category<>excluded.category OR magnanimous_connector_capability_absorption.native_target<>excluded.native_target OR magnanimous_connector_capability_absorption.boundary<>excluded.boundary OR magnanimous_connector_capability_absorption.source_kind<>excluded.source_kind OR magnanimous_connector_capability_absorption.status<>'tool-foundry-specified' OR magnanimous_connector_capability_absorption.research_json<>excluded.research_json OR magnanimous_connector_capability_absorption.spec_json<>excluded.spec_json;`);
...
ON CONFLICT(tenant_id,user_id,name) DO UPDATE SET purpose=excluded.purpose,family=excluded.family,inputs_json=excluded.inputs_json,outputs_json=excluded.outputs_json,steps_json=excluded.steps_json,risk=excluded.risk,status=CASE WHEN magnanimous_native_tool_specs.status='ready' THEN 'ready' ELSE excluded.status END,updated_at=excluded.updated_at WHERE magnanimous_native_tool_specs.purpose<>excluded.purpose OR magnanimous_native_tool_specs.family<>excluded.family OR magnanimous_native_tool_specs.inputs_json<>excluded.inputs_json OR magnanimous_native_tool_specs.outputs_json<>excluded.outputs_json OR magnanimous_native_tool_specs.steps_json<>excluded.steps_json OR magnanimous_native_tool_specs.risk<>excluded.risk OR (magnanimous_native_tool_specs.status<>'ready' AND magnanimous_native_tool_specs.status<>excluded.status);`);
...
ON CONFLICT(connector_id,capability_id) DO UPDATE SET tool_name=excluded.tool_name,native_target=excluded.native_target,mode=excluded.mode,status=excluded.status,native_route=excluded.native_route,evidence_module=excluded.evidence_module,boundary=excluded.boundary,requires_external=excluded.requires_external,proof_json=excluded.proof_json,updated_at=excluded.updated_at WHERE magnanimous_capability_realizations.tool_name<>excluded.tool_name OR magnanimous_capability_realizations.native_target<>excluded.native_target OR magnanimous_capability_realizations.mode<>excluded.mode OR magnanimous_capability_realizations.status<>excluded.status OR magnanimous_capability_realizations.native_route<>excluded.native_route OR magnanimous_capability_realizations.evidence_module<>excluded.evidence_module OR magnanimous_capability_realizations.boundary<>excluded.boundary OR magnanimous_capability_realizations.requires_external<>excluded.requires_external OR magnanimous_capability_realizations.proof_json<>excluded.proof_json;`);
 }
...
ON CONFLICT(id) DO UPDATE SET manifest_count=excluded.manifest_count,ledger_count=excluded.ledger_count,tool_spec_count=excluded.tool_spec_count,source_digest=excluded.source_digest,status='complete',updated_at=excluded.updated_at WHERE magnanimous_capability_materialization_state.manifest_count<>excluded.manifest_count OR magnanimous_capability_materialization_state.ledger_count<>excluded.ledger_count OR magnanimous_capability_materialization_state.tool_spec_count<>excluded.tool_spec_count OR magnanimous_capability_materialization_state.source_digest<>excluded.source_digest OR magnanimous_capability_materialization_state.status<>'complete';
...
ON CONFLICT(id) DO UPDATE SET manifest_count=excluded.manifest_count,native_ready_count=excluded.native_ready_count,hybrid_ready_count=excluded.hybrid_ready_count,bridge_required_count=excluded.bridge_required_count,specified_only_count=excluded.specified_only_count,source_digest=excluded.source_digest,status='complete',updated_at=excluded.updated_at WHERE magnanimous_capability_realization_state.manifest_count<>excluded.manifest_count OR magnanimous_capability_realization_state.native_ready_count<>excluded.native_ready_count OR magnanimous_capability_realization_state.hybrid_ready_count<>excluded.hybrid_ready_count OR magnanimous_capability_realization_state.bridge_required_count<>excluded.bridge_required_count OR magnanimous_capability_realization_state.specified_only_count<>excluded.specified_only_count OR magnanimous_capability_realization_state.source_digest<>excluded.source_digest OR magnanimous_capability_realization_state.status<>'complete';
`;
