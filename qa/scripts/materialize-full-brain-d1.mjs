...
WHERE magnanimous_connector_capability_absorption.connector_name IS NOT excluded.connector_name
 ...
 OR magnanimous_connector_capability_absorption.status IS NOT 'tool-foundry-specified'
 OR magnanimous_connector_capability_absorption.research_json IS NOT excluded.research_json
 OR magnanimous_connector_capability_absorption.spec_json IS NOT excluded.spec_json;`);
...
WHERE magnanimous_native_tool_specs.purpose IS NOT excluded.purpose
 OR magnanimous_native_tool_specs.family IS NOT excluded.family
 OR magnanimous_native_tool_specs.inputs_json IS NOT excluded.inputs_json
 OR magnanimous_native_tool_specs.outputs_json IS NOT excluded.outputs_json
 OR magnanimous_native_tool_specs.steps_json IS NOT excluded.steps_json
 OR magnanimous_native_tool_specs.risk IS NOT excluded.risk
 OR (magnanimous_native_tool_specs.status!='ready' AND magnanimous_native_tool_specs.status IS NOT excluded.status);`);
...
WHERE magnanimous_capability_realizations.tool_name IS NOT excluded.tool_name
 OR magnanimous_capability_realizations.native_target IS NOT excluded.native_target
 OR magnanimous_capability_realizations.mode IS NOT excluded.mode
 OR magnanimous_capability_realizations.status IS NOT excluded.status
 OR magnanimous_capability_realizations.native_route IS NOT excluded.native_route
 OR magnanimous_capability_realizations.evidence_module IS NOT excluded.evidence_module
 OR magnanimous_capability_realizations.boundary IS NOT excluded.boundary
 OR magnanimous_capability_realizations.requires_external IS NOT excluded.requires_external
 OR magnanimous_capability_realizations.proof_json IS NOT excluded.proof_json;`);
 }
...
WHERE magnanimous_capability_materialization_state.manifest_count IS NOT excluded.manifest_count
 OR magnanimous_capability_materialization_state.ledger_count IS NOT excluded.ledger_count
 OR magnanimous_capability_materialization_state.tool_spec_count IS NOT excluded.tool_spec_count
 OR magnanimous_capability_materialization_state.source_digest IS NOT excluded.source_digest
 OR magnanimous_capability_materialization_state.status IS NOT 'complete';
...
WHERE magnanimous_capability_realization_state.manifest_count IS NOT excluded.manifest_count
 OR magnanimous_capability_realization_state.native_ready_count IS NOT excluded.native_ready_count
 OR magnanimous_capability_realization_state.hybrid_ready_count IS NOT excluded.hybrid_ready_count
 OR magnanimous_capability_realization_state.bridge_required_count IS NOT excluded.bridge_required_count
 OR magnanimous_capability_realization_state.specified_only_count IS NOT excluded.specified_only_count
 OR magnanimous_capability_realization_state.source_digest IS NOT excluded.source_digest
 OR magnanimous_capability_realization_state.status IS NOT 'complete';
`;
