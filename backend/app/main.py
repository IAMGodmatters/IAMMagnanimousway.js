...
def startup():
 ...
 if c.execute("SELECT 1 FROM settings WHERE key='admin_password_hash'").fetchone() is None:
  c.execute("INSERT INTO settings(key,value) VALUES('admin_password_hash',?)",(hash_password(ADMIN_PASSWORD),)); c.execute("INSERT OR IGNORE INTO settings(key,value) VALUES('site_name','I AM Magnanimous AI Platform')"); c.execute("INSERT OR IGNORE INTO settings(key,value) VALUES('tagline','Free AI tools, Odin orchestration, and creator tools in one place.')"); c.execute("INSERT OR IGNORE INTO settings(key,value) VALUES('canva_url','')"); c.commit()
 c.close()
@app.get('/health')
...
def chat(req:ChatRequest):
 ...
 if not key:return {'output':'Odin is ready, but no cloud AI key is configured. Add an AI provider key to enable cloud AI. Local/free tools remain available.','provider':'local'}
 try:
