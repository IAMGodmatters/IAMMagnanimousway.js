...
: "${MAGNANIMOUS_STASIS_DIAGNOSTIC_EXTENSION:=6001}"
: "${ASTERISK_STASIS_APP:=magnanimous-call-control}"
...
export MAGNANIMOUS_SIP_DOMAIN MAGNANIMOUS_WEBRTC_EXTENSION MAGNANIMOUS_WEBRTC_ECHO_EXTENSION MAGNANIMOUS_STASIS_DIAGNOSTIC_EXTENSION ASTERISK_STASIS_APP MAGNANIMOUS_WEBRTC_PASSWORD MAGNANIMOUS_AI_EXTENSION MAGNANIMOUS_AI_PASSWORD
...
SUBST_VARS='${ASTERISK_ARI_USER} ${ASTERISK_ARI_PASSWORD} ${ASTERISK_SIP_PORT} ${ASTERISK_HTTPS_PORT} ${ASTERISK_WEBRTC_ENABLED} ${ASTERISK_TLS_CERT_FILE} ${ASTERISK_TLS_KEY_FILE} ${ASTERISK_STUN_SERVER} ${MAGNANIMOUS_WEBRTC_EXTENSION} ${MAGNANIMOUS_WEBRTC_ECHO_EXTENSION} ${MAGNANIMOUS_STASIS_DIAGNOSTIC_EXTENSION} ${ASTERISK_STASIS_APP} ${MAGNANIMOUS_WEBRTC_PASSWORD} ${MAGNANIMOUS_SIP_DOMAIN} ${MAGNANIMOUS_AI_EXTENSION} ${MAGNANIMOUS_AI_PASSWORD} ${MAGNANIMOUS_ADMIN_EXTENSION} ${MAGNANIMOUS_ADMIN_PASSWORD} ${CARRIER_SIP_ENDPOINT} ${CARRIER_SIP_HOST} ${CARRIER_SIP_PORT} ${CARRIER_SIP_USERNAME} ${CARRIER_SIP_PASSWORD} ${CARRIER_SIP_FROM_DOMAIN} ${CARRIER_SIP_SECONDARY_ENDPOINT} ${CARRIER_SIP_SECONDARY_HOST} ${CARRIER_SIP_SECONDARY_PORT} ${CARRIER_SIP_SECONDARY_USERNAME} ${CARRIER_SIP_SECONDARY_PASSWORD} ${CARRIER_SIP_SECONDARY_FROM_DOMAIN} ${TELECOM_PUBLIC_IP}'
