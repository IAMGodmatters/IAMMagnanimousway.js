...
function replaceOnce(text,needle,replacement,label){
 ...
 const i=text.indexOf(needle);if(i<0)throw new Error(`Missing patch anchor: ${label}`);
...
// Allow Porkbun credentials to flow from the encrypted owner vault/bootstrap into runtime adapters.
{
...
// Add Porkbun to the existing encrypted Provider Vault UI/API rather than creating a second secret store.
{
...
// Make DNS/domain operations visible in the central brain capability inventory too.
{
...
// Synchronize optional GitHub/Cloudflare secrets, while still supporting the encrypted in-app vault.
{
 const p='.github/workflows/deploy.yml';let s=read(p);
 s=replaceOnce(s,"          MISTRAL_API_KEY: ${{ secrets.MISTRAL_API_KEY }}\n          INTEGRATION_CREDENTIALS_KEY:","          MISTRAL_API_KEY: ${{ secrets.MISTRAL_API_KEY }}\n          PORKBUN_API_KEY: ${{ secrets.PORKBUN_API_KEY }}\n          PORKBUN_SECRET_API_KEY: ${{ secrets.PORKBUN_SECRET_API_KEY }}\n          INTEGRATION_CREDENTIALS_KEY:",'deploy Porkbun env');
 s=replaceOnce(s,'          sync_secret MISTRAL_API_KEY "$MISTRAL_API_KEY"\n          sync_secret INTEGRATION_CREDENTIALS_KEY', '          sync_secret MISTRAL_API_KEY "$MISTRAL_API_KEY"\n          sync_secret PORKBUN_API_KEY "$PORKBUN_API_KEY"\n          sync_secret PORKBUN_SECRET_API_KEY "$PORKBUN_SECRET_API_KEY"\n          sync_secret INTEGRATION_CREDENTIALS_KEY','deploy Porkbun secret sync');
...
// Environment documentation for operators who prefer protected runtime secrets over the in-app vault.
{
...
for(const p of ['scripts/finish-dns-integration.mjs','.github/workflows/finish-dns-integration.yml']){try{fs.unlinkSync(p);}catch{}}
