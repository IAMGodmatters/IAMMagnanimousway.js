...
async function fetchPublic(value,{redirects=0,maxBytes=5*1024*1024}={}){
 ...
 return await new Promise((resolve,reject)=>{
  const request=transport.request({
   ...
   headers:{
    host:target.host,
    'user-agent':'MagnanimousBrowser/1.0',
    accept:'text/html,application/xhtml+xml,text/plain,image/*,*/*;q=0.5',
   ...
   response.on('end',()=>{
    ...
    resolve({
     url:target.toString(),
     status,
     content_type:String(response.headers['content-type']||'application/octet-stream').split(';')[0].trim().toLowerCase(),
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='GET'&&requestUrl.pathname==='/health')return json(res,200,{ok:true,identity:'Magnanimous Browser Egress',private_network_targets:false,redirect_revalidation:true});
