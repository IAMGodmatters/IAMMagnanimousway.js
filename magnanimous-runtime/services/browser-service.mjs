...
async function render(spec={}){
 ...
 try{
  ...
  const r=await run([...common,flag,target],{timeout:spec.timeout_ms,env:browserEnv});
  if(r.code!==0)throw new Error('Chromium snapshot render failed: '+r.stderr.toString('utf8').slice(-1200));
...
const server=http.createServer(async(req,res)=>{
 try{
  if(req.url==='/health')return json(res,200,{ok:true,identity:'Magnanimous Browser',engine:'Chromium snapshot renderer',private_network_targets:false,egress_proxy_required:true,egress_proxy_configured:Boolean(proxy),network_mode:'safe-egress-snapshot'});
