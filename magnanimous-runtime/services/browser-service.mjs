...
async function fetchSnapshot(url){
 ...
 for(let attempt=0;attempt<3;attempt++){
  try{
   ...
   await ensureProxyReady(4);
  }
 }
 throw new Error('Magnanimous browser snapshot fetch failed: '+lastError);
}
...
async function render(spec={}){
 ...
 if(mode==='dom'){
  ...
  return{ok:true,mode,url:snapshot.url||url,status:Number(snapshot.status||200),content_type:type,html:bytes.toString('utf8').slice(0,2000000),bytes:Number(snapshot.bytes||bytes.length),snapshot_mode:true};
 }
