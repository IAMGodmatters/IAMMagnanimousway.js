...
const GENERIC_MESSAGE='Use a Magnanimous recovery code or a device where you are still signed in to continue account recovery.';
const INVALID_TOKEN_MESSAGE='This reset session is invalid or expired. Start account recovery again.';
...
async function activeSessionUser(request,env){
 try{return await currentUser(request,env)}catch{return null}
}
function sameUser(left,right){
 return Boolean(left&&right&&String(left.id||'')===String(right.id||'')&&String(left.tenant_id||'')===String(right.tenant_id||''));
}
async function issueNativeReset(env,user,source){
 ...
 return{
  ...
  recovery_method:String(source||'magnanimous-native'),
  detail:'Identity verified by Magnanimous. Create your new password now.'
 };
}
...
async function requestReset(request,env){
 ...
 if(!email||!validEmail(email))return json({ok:true,detail:GENERIC_MESSAGE,recovery_required:true});
 ...
 if(!user){await logAuth(env,null,'password_reset_requested',1,email);return json({ok:true,detail:GENERIC_MESSAGE,recovery_required:true})}
 ...
 if(recoveryCode){
  ...
  const row=await env.DB.prepare(`SELECT id FROM account_recovery_codes
    WHERE user_id=? AND tenant_id=? AND code_hash=? AND used_at IS NULL LIMIT 1`).bind(user.id,user.tenant_id,codeHash).first();
  if(!row){await logAuth(env,user,'password_recovery_code_failed',0,email);return json({detail:'That Magnanimous recovery code is invalid or already used.',code:'RECOVERY_CODE_INVALID'},400)}
  const claimed=await env.DB.prepare('UPDATE account_recovery_codes SET used_at=? WHERE id=? AND used_at IS NULL').bind(now(),row.id).run();
 ...
 await logAuth(env,user,'password_reset_native_recovery_required',1,email);
 return json({
  ok:true,
  detail:GENERIC_MESSAGE,
  recovery_required:true,
  methods:['trusted_session','recovery_code']
 });
}
...
async function recoveryCodeStatus(request,env){
 ...
 const user=await activeSessionUser(request,env);
...
async function rotateRecoveryCodes(request,env){
 ...
 for(let i=0;i<hashes.length;i+=1){
  await env.DB.prepare('INSERT INTO account_recovery_codes(user_id,tenant_id,code_hash,created_at,used_at) VALUES(?,?,?,?,NULL)').bind(user.id,user.tenant_id,hashes[i],t).run();
 }
 ...
 return json({
  ...
  codes,
  count:codes.length,
  detail:'New Magnanimous recovery codes created. Each code works once. Save them somewhere private; the platform stores only their hashes.'
 });
}
...
async function completeReset(request,env){
 ...
 if(Number(changed?.meta?.changes||0)!==1)return json({detail:'Your password could not be changed safely. Start account recovery again.',code:'PASSWORD_RESET_FAILED'},409);
