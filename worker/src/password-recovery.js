...
async function requestReset(request,env){
 ...
 // Keep any previously delivered reset link valid until the replacement email
 // is actually accepted by a mail transport. A failed resend must never strand
 // the account by invalidating the user's last working link.
