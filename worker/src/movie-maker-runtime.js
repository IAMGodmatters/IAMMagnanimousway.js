...
async function pollStudioVideo(request,env,user,jobId){
 ...
 const persisted=await persistAsset(env,request,user,{kind:'video',title:job.title,bytes,content_type:contentType,watermarked:false,origin:priced.provider_origin_cost_usd,customer:variable.customer_charge_usd,source_text:job.prompt});
