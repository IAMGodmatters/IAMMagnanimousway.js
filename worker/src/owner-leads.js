...
export async function handleOwnerLeads(request, env) {
  ...
  const query = `
    SELECT
      ...
      (
        ...
        FROM auth_events a
        WHERE a.user_id = u.id AND a.success = 1 AND a.event IN ('login','signup')
      ) AS last_activity,
      (
        SELECT COUNT(*)
        FROM auth_events a
        WHERE a.user_id = u.id AND a.success = 1 AND a.event = 'login'
      ) AS login_count
    FROM users u
  ...
  const recentRow = await env.DB.prepare("SELECT COUNT(*) AS total FROM users WHERE role <> 'owner' AND created_at>=?").bind(todayStart).first();
