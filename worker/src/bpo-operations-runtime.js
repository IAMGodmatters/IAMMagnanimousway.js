...
export async function handleBpoOperations(request,env){
 ...
 try{
  await ensure(env);const user=await currentUser(request,env);if(!user)return json({detail:'Sign in to use BPO operations.'},401);const tenant=String(user.tenant_id),owner=['owner','admin'].includes(String(user.role||'').toLowerCase());
  ...
  const clientMatch=url.pathname.match(/^\/api\/bpo\/clients\/([^/]+)$/);
  if(clientMatch&&request.method==='PATCH'){
   if(!owner)return json({detail:'Workspace owner access required.'},403);
   const current=await env.DB.prepare('SELECT * FROM bpo_clients WHERE id=? AND tenant_id=?').bind(clientMatch[1],tenant).first();if(!current)return json({detail:'Client not found.'},404);
   const b=await request.json().catch(()=>({})),status=['active','archived','paused'].includes(String(b.status||''))?String(b.status):current.status;
   await env.DB.prepare('UPDATE bpo_clients SET name=?,industry=?,service_lines=?,status=?,data_classification=?,notes=?,updated_at=? WHERE id=? AND tenant_id=?').bind(text(b.name===undefined?current.name:b.name,160),text(b.industry===undefined?current.industry:b.industry,120),b.service_lines===undefined?current.service_lines:JSON.stringify(arr(b.service_lines).map(x=>text(x,80)).filter(Boolean)),status,text(b.data_classification===undefined?current.data_classification:b.data_classification,40),text(b.notes===undefined?current.notes:b.notes),now(),clientMatch[1],tenant).run();
   await audit(env,tenant,'client.updated',{client_id:clientMatch[1],actor_id:user.id,detail:`${current.status} -> ${status}`});return json({ok:true,status});
  }
