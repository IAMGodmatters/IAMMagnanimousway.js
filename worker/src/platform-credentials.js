...
export const PLATFORM_CREDENTIAL_GROUPS=[
 ...
 {id:'carrier-bridge',name:'Bring Your Own Carrier / SIP Bridge',providers:['carrier-bridge'],fields:[
  ...
  {key:'VOIP_BYOC_ROUTE_CONTRACT',label:'Planner Route Contract (use magnanimous-route-v1 only when the bridge supports explicit route keys)',secret:false,required:false}
 ]},
 ...
 {id:'agent-brains',name:'Agent Mesh AI Brains',providers:['google-ai','groq','openrouter-free','huggingface','mistral','cerebras','nvidia-nim','xai-grok'],fields:[
  ...
  {key:'XAI_API_KEY',label:'xAI API Key (optional metered Grok compute adapter)',secret:true,required:false},
  {key:'XAI_MODEL',label:'xAI model override (optional; default grok-4.6)',secret:false,required:false}
 ]},
...
export function runtimeEnvOverlay(base,additions={}){const overlay=Object.create(base||null);for(const[key,value]of Object.entries(additions||{}))overlay[key]=value;return overlay}
...
export async function getIntegrationRuntimeEnv(env){if(!env?.DB)return env;try{const rows=await vaultRows(env);if(!rows.length)return env;const merged=runtimeEnvOverlay(env);for(const row of rows){if(!ALLOWED_KEYS.has(row.credential_key))continue;if(typeof merged[row.credential_key]==='string'&&merged[row.credential_key].trim())continue;merged[row.credential_key]=await decrypt(row.encrypted_value,env)}return merged}catch(error){console.error('platform credential runtime load failed',error);return env}}
...
export async function handlePlatformCredentials(request,env){
 ...
 if(request.method==='POST'){const b=await request.json().catch(()=>({}));try{return json(await setPlatformCredential(env,user,String(b.key||'').trim(),String(b.value||'')))}catch(error){return json({detail:String(error?.message||error)},400)}}
 if(request.method==='DELETE'){try{return json(await deletePlatformCredential(env,user,String(url.searchParams.get('key')||'').trim()))}catch(error){return json({detail:String(error?.message||error)},400)}}
