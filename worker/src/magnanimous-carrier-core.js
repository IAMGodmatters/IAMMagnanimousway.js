...
export async function handleMagnanimousCarrierCore(request,env){
 ...
 if(request.method==='POST'&&path==='/api/magnanimous/carrier/routes'){
  ...
  if(bridgeRouteKey&&!/^[A-Za-z0-9_.:-]{1,120}$/.test(bridgeRouteKey))return json({detail:'bridge_route_key must be a stable non-secret route key.'},422);
  if(bridgeRouteKey&&String(ic.type||'')!=='byoc-bridge')return json({detail:'Only a byoc-bridge interconnect can declare bridge_route_key.'},422);
  if(executionEndpoint&&bridgeRouteKey)return json({detail:'A route cannot use both asterisk_endpoint and bridge_route_key.'},422);
  if(executionEndpoint){policy.asterisk_endpoint=executionEndpoint;delete policy.endpoint_key}
  if(bridgeRouteKey)policy.bridge_route_key=bridgeRouteKey;
  ...
  return json({ok:true,id:r.meta?.last_row_id||null,selected_route_execution:executionEndpoint?'magnanimous-telecom-core':bridgeRouteKey?'generic-byoc-contract':'preview-only'},201);
 }
