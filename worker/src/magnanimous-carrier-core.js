...
async function routePlan(env,user,to,mode='balanced'){
 ...
 const matches=results.filter(x=>!x.destination_prefix||destination.startsWith(String(x.destination_prefix))).map(x=>{
  ...
  const bridgeRouteKey=String(policy.byoc_route_key||policy.bridge_route_key||'').trim().slice(0,120);
...
export async function handleMagnanimousCarrierCore(request,env){
 ...
 if(request.method==='POST'&&path==='/api/magnanimous/carrier/routes'){
  ...
  const bridgeRouteKey=String(policy.byoc_route_key||policy.bridge_route_key||'').trim();
  ...
  if(bridgeRouteKey&&!/^[A-Za-z0-9_.:-]{1,120}$/.test(bridgeRouteKey))return json({detail:'byoc_route_key must be an opaque workspace bridge route key, not a URL or credential.'},422);
  if(bridgeRouteKey&&String(ic.type||'')!=='byoc-bridge')return json({detail:'Only a byoc-bridge interconnect can declare byoc_route_key.'},422);
  if(executionEndpoint&&bridgeRouteKey)return json({detail:'Choose either asterisk_endpoint or byoc_route_key for one route, not both.'},422);
  if(executionEndpoint){policy.asterisk_endpoint=executionEndpoint;delete policy.endpoint_key}
  if(bridgeRouteKey){policy.byoc_route_key=bridgeRouteKey;delete policy.bridge_route_key}
  ...
  return json({ok:true,id:r.meta?.last_row_id||null,selected_route_execution:executionEndpoint?'magnanimous-telecom-core':bridgeRouteKey?'workspace-byoc':'preview-only'},201);
 }
