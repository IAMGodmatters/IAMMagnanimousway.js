...
export async function handleWellness(request,env){
 ...
 if(request.method==='POST'&&url.pathname==='/api/wellness/checkins'){
  const b=await request.json().catch(()=>({}));await env.DB.prepare('INSERT INTO wellness_checkins(tenant_id,user_id,mood,energy,sleep_hours,water_cups,movement_minutes,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(tenant,uid,Math.max(0,Math.min(10,Number(b.mood||0))),Math.max(0,Math.min(10,Number(b.energy||0))),Math.max(0,Math.min(24,Number(b.sleep_hours||0))),Math.max(0,Math.min(100,Number(b.water_cups||0))),Math.max(0,Math.min(1440,Number(b.movement_minutes||0))),clip(b.notes,2000),now()).run();return json({ok:true});
 }
