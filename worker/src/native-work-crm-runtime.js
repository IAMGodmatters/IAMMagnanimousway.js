...
export async function handleNativeWorkCrm(request,env){
 ...
 const workforce=await handleWorkforceOps(request,env,user,body);if(workforce)return workforce;
 const suite=await handleMagnanimousSuite(request,env,user,body);if(suite)return suite;
