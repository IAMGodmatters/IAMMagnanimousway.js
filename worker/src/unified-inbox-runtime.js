...
import {deliverUnifiedInboxMessage,messagingReadiness} from './white-label-message-transport.js';
...
async function stopCrmSequencesOnInbound(env,tenant,contactId,threadId=''){
 if(!contactId)return 0;const ts=now();let stopped=0;
 try{
  const{results=[]}=await env.DB.prepare("SELECT e.id,e.sequence_id,s.settings_json FROM crm_sequence_enrollments e JOIN magnanimous_ops_sequences s ON s.id=e.sequence_id AND s.tenant_id=e.tenant_id WHERE e.tenant_id=? AND e.contact_id=? AND e.status='active'").bind(tenant,Number(contactId)).all();
  for(const row of results){let settings={};try{settings=JSON.parse(row.settings_json||'{}')}catch{}if(settings.stop_on_reply===false)continue;
   await env.DB.prepare("UPDATE crm_sequence_enrollments SET status='goal_met',next_due_at=NULL,updated_at=? WHERE tenant_id=? AND id=?").bind(ts,tenant,row.id).run();
   try{await env.DB.prepare("UPDATE crm_activities SET completed=1 WHERE tenant_id=? AND sequence_enrollment_id=? AND completed=0").bind(tenant,row.id).run()}catch{}
   try{await env.DB.prepare("INSERT INTO crm_sequence_events(tenant_id,enrollment_id,sequence_id,contact_id,step_index,event_type,status,detail_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)").bind(tenant,row.id,row.sequence_id,Number(contactId),-1,'goal.reply','goal_met',JSON.stringify({thread_id:threadId}),ts).run()}catch{}
   stopped++;
  }
 }catch{}
 return stopped;
...
async function overview(env,tenant){
 ...
 return{ok:true,open,waiting,unassigned,urgent,bpo_open_items:bpoOpen,channels:['email','sms','voice','voicemail','social','chat','web','task'],capture_endpoint:'/api/inbox/capture',handoff:'Magnanimous AI + named specialists + human users',delivery:messagingReadiness(env)};
}
...
export async function handleUnifiedInbox(request,env){
 ...
 try{
  ...
  if(request.method==='POST'&&(url.pathname==='/api/inbox/threads'||url.pathname==='/api/inbox/capture')){
   ...
   await audit(env,tenant,id,'thread.created',user,`${text(b.channel||'task',40)} • ${subject}`);const direction=text(b.direction||'inbound',20);const stopped=direction==='inbound'&&crmContactId?await stopCrmSequencesOnInbound(env,tenant,crmContactId,id):0;return json({ok:true,id,sequence_goals_met:stopped},201)
  }
  ...
  if(m&&request.method==='POST'){
   const thread=await env.DB.prepare('SELECT * FROM unified_inbox_threads WHERE id=? AND tenant_id=?').bind(m[1],tenant).first();if(!thread)return json({detail:'Inbox thread not found.'},404);const b=await request.json().catch(()=>({})),content=text(b.content||b.message,30000);if(!content)return json({detail:'Reply content is required.'},400);const ts=now(),direction=text(b.direction||'outbound',20);
   let delivery={ok:true,stored_only:true,receipt:'inbox-only'};if(direction==='outbound'&&b.send_external!==false&&['email','sms'].includes(String(thread.channel||'').toLowerCase())){delivery=await deliverUnifiedInboxMessage(env,{tenant,thread,content,subject:thread.subject,consentConfirmed:b.consent_confirmed===true});if(!delivery.ok)return json({detail:delivery.error||'External delivery failed.',code:delivery.code||'DELIVERY_FAILED',delivery_channel:thread.channel},Number(delivery.status)||502)}
   const metadata={...(b.metadata||{}),delivery:{channel:delivery.channel||thread.channel,receipt:delivery.receipt||'',stored_only:Boolean(delivery.stored_only)}};const result=await env.DB.prepare('INSERT INTO unified_inbox_messages(thread_id,tenant_id,direction,author_type,author_name,content,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?)').bind(m[1],tenant,direction,text(b.author_type||'user',40),text(b.author_name||user.name||user.email,160),content,JSON.stringify(metadata),ts).run();await env.DB.prepare('UPDATE unified_inbox_threads SET last_message_at=?,updated_at=?,status=? WHERE id=? AND tenant_id=?').bind(ts,ts,text(b.thread_status||'waiting',30),m[1],tenant).run();await audit(env,tenant,m[1],delivery.stored_only?'message.saved':'message.delivered',user,text(content,240));const stopped=direction==='inbound'&&thread.crm_contact_id?await stopCrmSequencesOnInbound(env,tenant,Number(thread.crm_contact_id),m[1]):0;return json({ok:true,id:result.meta?.last_row_id||null,sequence_goals_met:stopped,delivery:{channel:delivery.channel||thread.channel,receipt:delivery.receipt||'',stored_only:Boolean(delivery.stored_only)}},201)
  }
