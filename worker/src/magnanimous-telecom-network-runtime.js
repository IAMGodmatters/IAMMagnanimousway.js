...
const MOBILE_PARTNERS=[
 ...
 {key:'telna',name:'Telna',status:'contacted',channel:'email',destination:'bd@telna.com',case_reference:'',evidence_reference:'gmail:1a0db5025ca68532;gmail:1a0dbdbf2f5daf4a',sandbox_reference:'https://www.telna.com/faqs',notes:'Telna was followed up at bd@telna.com with info@telna.com copied after current public materials confirmed CSP/white-label consumer eSIM, API management, trial SIM availability, usage/spend control and 800+ network / 200+ country reach. Magnanimous specifically requested Philippines network/IMSI eligibility, no-cost trial connectivity, wholesale economics, compliance allocation and genuinely independent backup-network proof. No sandbox, country eligibility, wholesale price or live profile is inferred until Telna supplies it.',next_action:'Await Telna commercial response with Philippines network/IMSI list, white-label/reseller agreement, sandbox/API or trial access, wholesale rate deck/fair-use, KYC/SIM-registration obligations and independent primary/backup network details.'},
 {key:'1global',name:'1GLOBAL',status:'case_followed_up',channel:'email',destination:'business.help@1global.com',case_reference:'02547094',evidence_reference:'gmail:1a0db5066b33f92d;gmail:1a0dbc4f2e590b07;gmail:1a0dbc53906621d2;gmail:1a0dbcee2745d3d6;gmail:1a0dbcf10d0bce25',sandbox_reference:'https://docs.connect-api.1global.com/overview/getstarted/',notes:'1GLOBAL sent repeated automated case-updated notices without the requested commercial/technical evidence. Magnanimous replied once with the full substantive request and suppresses duplicate auto-ack replies to avoid an email loop while waiting for a human Connect / Embedded Telco owner.',next_action:'Keep case 02547094 open and await a substantive human Connect / Embedded Telco response with platform credentials/sandbox terms, commercial agreement, Philippines coverage/compliance, pricing, real trial eSIM path and independent backup details.'},
 {key:'fonus',name:'Fonus',status:'sales_handoff',channel:'official-reseller-form+direct-email',destination:'zhac@fonusmobile.com',case_reference:'',evidence_reference:'tinyfish-run:6e18befa-5503-43c4-8100-1c3807c14805;gmail:1a0db65376739729;gmail:1a0db668aabea67c;gmail:1a0db67a03572b70;gmail:1a0dbc4fa825d47f;gmail:1a0dbcecffc2054a;gmail:1a0dbd4cd3dd3738',sandbox_reference:'',notes:'Fonus Support explicitly confirmed that Zhac will handle the reseller concern. Magnanimous acknowledged the handoff once and is now awaiting Zhac for agreement-grade resale authority, wholesale pricing, provisioning/API, Philippines eligibility/compliance and real trial connectivity terms.',next_action:'Await Zhac direct commercial response; capture agreement, rate deck, Philippines eligibility, provisioning/profile method, trial connectivity and genuinely independent backup-network terms.'},
 {key:'pldt-smart',name:'PLDT Enterprise / Smart',status:'contacted',channel:'official-business-email',destination:'pldtenterprise@pldt.com.ph',case_reference:'',evidence_reference:'gmail:1a0dbe42b953cad0',sandbox_reference:'https://pldtenterprise.com/global/enterprise/contact-us',notes:'Official PLDT Enterprise business-development inquiry sent requesting a Philippine host-network/reseller/MVNO/VNO path, NTC-required carrier agreement, SIM/eSIM provisioning/API, voice/SMS/data/number/porting scope, SIM Registration Act allocation, wholesale economics, trial profile and independent-backup guidance. No paid activation or deposit authorized.',next_action:'Await PLDT/Smart business-development response and capture host/reseller agreement path, Philippine provisioning/API, trial profile, wholesale pricing, compliance allocation and backup-network requirements.'},
 {key:'dito',name:'DITO Telecommunity',status:'contacted',channel:'official-partner-email',destination:'partner.inquiry@dito.ph',case_reference:'',evidence_reference:'gmail:1a0dbe434a054d64',sandbox_reference:'https://dito.ph/partners',notes:'Official DITO partner inquiry sent requesting a Philippine host-network/branded-reseller/MVNO/VNO path, NTC-required agreement, SIM/eSIM provisioning/API, voice/SMS/data/number/porting scope, SIM Registration Act allocation, wholesale economics, trial profile, branding terms and independent-backup guidance. No paid activation or deposit authorized.',next_action:'Await DITO partner response and capture host/reseller agreement path, Philippine provisioning/API, trial profile, wholesale pricing, compliance allocation and backup-network requirements.'}
];
...
async function globalMobileReadiness(env,tenant){
 ...
 try{
  ...
  const ts=now();
  offer=await env.DB.prepare("SELECT id,adapter_key,network_group,country_code,origin_reference,commercial_reference,funded_variable_cost_cap,origin_variable_cost_per_gb FROM telecom_mobile_wholesale_offers WHERE tenant_id=? AND status='active' AND origin_cost_verified=1 AND commercial_authorized=1 AND commercial_reference<>'' AND country_verified=1 AND data_supported=1 AND origin_reference<>'' AND (origin_variable_cost_per_gb=0 OR funded_variable_cost_cap>0) AND (valid_from IS NULL OR valid_from<=?) AND (valid_to IS NULL OR valid_to>=?) ORDER BY updated_at DESC LIMIT 1").bind(tenant,ts,ts).first();
...
export async function handleMagnanimousTelecomNetwork(request,env){
 ...
 if(path==='/api/telecom/network/global-mobile/offers'&&request.method==='POST'){
  ...
  await event(env,tenant,user,'global_mobile.wholesale_offer.activated',adapterKey,country,{offer_id:offerId,origin_reference:originReference,commercial_reference:commercialReference});
