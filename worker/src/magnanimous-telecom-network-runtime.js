 ...
 if(path==='/api/telecom/network/overview'&&request.method==='GET'){
  ...
  return json({
   ...
   routing_policy:{
    ...
    selected_route_execution:{native_telecom_core:true,generic_byoc_contract_gated:Boolean(truthy(env?.VOIP_PROVIDER_ROUTE_CONTROL_ENABLED)&&String(env?.VOIP_PROVIDER_HEALTH_URL||'').trim()),twilio_compatibility:true,plivo_compatibility:true},
    execution_note:'Explicit Twilio and Plivo route types execute only when selected by the Magnanimous planner and pass authenticated account-health checks. The protected Magnanimous Telecom Core keeps its authenticated Asterisk route gate. Generic BYOC can also execute an explicit planner route only through the owner-enabled same-origin HTTPS selected-route contract with pre-call health and exact post-call confirmation. No-route legacy compatibility fallback remains outside full planner authority, so all-live-route authority remains false.',
