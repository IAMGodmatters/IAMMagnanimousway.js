 ...
 if(path==='/api/telecom/network/overview'&&request.method==='GET'){
  ...
  return json({
   ...
   routing_policy:{
    ...
    selected_route_execution:{native_telecom_core:true,generic_byoc_contract:true,generic_byoc_legacy_fallback:true,twilio_compatibility:true,plivo_compatibility:true},
    execution_note:'Explicit Twilio/Plivo routes and the protected Magnanimous Telecom Core are planner-controlled. Generic BYOC can opt into the same authority only through the same-origin HTTPS magnanimous.carrier-route.v1 contract with an opaque route key and bridge confirmation. Existing non-opted-in generic BYOC and no-route legacy compatibility fallbacks remain available, so all-live-route authority remains false.',
