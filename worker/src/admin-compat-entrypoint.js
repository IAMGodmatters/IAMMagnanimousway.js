  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/admin/')) {
        ...
        if (url.pathname === '/api/admin/ads' && request.method === 'POST') { const b = await request.json(); try{await env.DB.prepare('ALTER TABLE ads ADD COLUMN revenue_authorized INTEGER NOT NULL DEFAULT 0').run()}catch(_){} try{await env.DB.prepare('ALTER TABLE ads ADD COLUMN owner_owned INTEGER NOT NULL DEFAULT 0').run()}catch(_){} const r = await env.DB.prepare('INSERT INTO ads(title,url,label,placement,active,created_at,revenue_authorized,owner_owned) VALUES(?,?,?,?,?,?,0,1)').bind(String(b.title||''),String(b.url||''),String(b.label||'Magnanimous promotion'),String(b.placement||'home'),b.active?1:0,now()).run(); return json({ id:r.meta.last_row_id },201); }
