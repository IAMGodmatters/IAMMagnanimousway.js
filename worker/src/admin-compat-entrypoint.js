  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/admin/')) {
        ...
        if (url.pathname === '/api/admin/ads' && request.method === 'GET') { const { results } = await env.DB.prepare('SELECT id,title,url,label,placement,active,owner_owned,revenue_approved FROM ads ORDER BY id DESC').all(); return json({ ads: results }); }
        if (url.pathname === '/api/admin/ads' && request.method === 'POST') { const b = await request.json(); const ownerOwned=b.owner_owned===true; const revenueApproved=b.revenue_approved===true; const active=b.active===true&&(ownerOwned||revenueApproved); const r = await env.DB.prepare('INSERT INTO ads(title,url,label,placement,active,owner_owned,revenue_approved,created_at) VALUES(?,?,?,?,?,?,?,?)').bind(String(b.title||''),String(b.url||''),String(b.label||'Sponsored'),String(b.placement||'home'),active?1:0,ownerOwned?1:0,revenueApproved?1:0,now()).run(); return json({ id:r.meta.last_row_id,active,owner_owned:ownerOwned,revenue_approved:revenueApproved },201); }
        const m=url.pathname.match(/^\/api\/admin\/ads\/(\d+)$/); if(m&&request.method==='PUT'){const b=await request.json();const ownerOwned=b.owner_owned!==false,revenueApproved=b.revenue_approved===true,active=b.active===true&&(ownerOwned||revenueApproved);await env.DB.prepare('UPDATE ads SET title=?,url=?,label=?,placement=?,active=?,owner_owned=?,revenue_approved=? WHERE id=?').bind(String(b.title||''),String(b.url||''),String(b.label||'Sponsored'),String(b.placement||'home'),active?1:0,ownerOwned?1:0,revenueApproved?1:0,Number(m[1])).run();return json({ok:true,active,owner_owned:ownerOwned,revenue_approved:revenueApproved});} if(m&&request.method==='DELETE'){await env.DB.prepare('DELETE FROM ads WHERE id=?').bind(Number(m[1])).run();return json({ok:true});}
      }
