...
export async function handleWhiteLabelPayments(request,rawEnv){
 ...
 if(url.pathname==='/api/white-label/payments/confirm'&&request.method==='POST'){
  ...
  const r=await stripe(env,`/v1/checkout/sessions/${encodeURIComponent(sid)}`,{account:record.connected_account_id});if(!r.ok)return json({detail:clean(r.data?.error?.message||'Client payment could not be verified.')},502);const amountMatches=Number(r.data?.amount_total||0)===Number(record.amount_cents||0),currencyMatches=String(r.data?.currency||'').toLowerCase()===String(record.currency||'').toLowerCase(),paid=String(r.data?.status||'')==='complete'&&String(r.data?.payment_status||'')==='paid'&&amountMatches&&currencyMatches,ts=now(),verifiedStatus=paid?'paid':(!amountMatches||!currencyMatches)?'verification_failed':String(r.data?.payment_status||r.data?.status||'pending');
  await env.DB.prepare('UPDATE agency_client_charge_sessions SET status=?,updated_at=? WHERE id=? AND tenant_id=?').bind(verifiedStatus,ts,record.id,tenant).run();if(paid&&record.usage_id)await env.DB.prepare("UPDATE agency_usage_rebill SET status='paid',updated_at=? WHERE id=? AND tenant_id=?").bind(ts,record.usage_id,tenant).run();
  return json({confirmed:paid,status:verifiedStatus,amount_verified:amountMatches,currency_verified:currencyMatches,usage_id:record.usage_id||null});
 }
...
export async function applyWhiteLabelClientPaymentWebhook(rawEnv,event){
 ...
 const amountMatches=Number(object?.amount_total||0)===Number(record.amount_cents||0),currencyMatches=String(object?.currency||'').toLowerCase()===String(record.currency||'').toLowerCase(),paid=type!=='checkout.session.async_payment_failed'&&String(object?.payment_status||'').toLowerCase()==='paid'&&amountMatches&&currencyMatches,ts=now(),status=paid?'paid':(!amountMatches||!currencyMatches)?'verification_failed':type==='checkout.session.async_payment_failed'?'payment_failed':clean(object?.payment_status||object?.status||'pending',40);
