...
async function saveConnection(env,tenant,acct){
 ...
 .bind(tenant,clean(acct.id,100),acct.charges_enabled?'ready':acct.details_submitted?'restricted':'onboarding',acct.charges_enabled?1:0,acct.payouts_enabled?1:0,acct.details_submitted?1:0,clean(acct.country,10),clean(acct.default_currency,10),ts,ts).run();
...
export async function handleWhiteLabelPayments(request,env){
 ...
 if(url.pathname==='/api/white-label/payments/onboard'&&request.method==='POST'){
  if(!owner(user))return json({detail:'Workspace owner or admin access required.'},403);if(!env.STRIPE_SECRET_KEY)return json({detail:'Stripe Connect API access is not configured for this platform.',code:'STRIPE_CONNECT_NOT_CONFIGURED'},503);
  let row=await connection(env,tenant),accountId=clean(row?.stripe_account_id,100);
  if(!accountId){
   const form=new URLSearchParams();form.set('metadata[tenant_id]',tenant);form.set('metadata[platform_feature]','white_label_client_payments');
   const created=await stripe(env,'/v1/accounts',{method:'POST',body:form});if(!created.ok||!created.data?.id)return json({detail:clean(created.data?.error?.message||'Stripe could not create the connected account.')},created.status>=400&&created.status<500?created.status:502);
   accountId=String(created.data.id);row=await saveConnection(env,tenant,created.data);
  }else row=await refresh(env,tenant,row);
  ...
  form.set('collection_options[fields]','eventually_due');const link=await stripe(env,'/v1/account_links',{method:'POST',body:form});if(!link.ok||!link.data?.url)return json({detail:clean(link.data?.error?.message||'Stripe onboarding could not start.')},link.status>=400&&link.status<500?link.status:502);
  return json({ok:true,onboarding_url:link.data.url,expires_at:link.data.expires_at||null,connection:publicConnection(row),note:'Stripe-hosted onboarding collects identity/business/bank requirements. Magnanimous does not store card or bank credentials.'});
 }
 if(url.pathname==='/api/white-label/payments/usage-checkout'&&request.method==='POST'){
  ...
  const description=clean(`${usage.client_name} • ${usage.category} • ${usage.period}`,240),sessionId=crypto.randomUUID(),form=new URLSearchParams();
  ...
  const ts=now();await env.DB.prepare('INSERT INTO agency_client_charge_sessions(id,tenant_id,client_id,usage_id,connected_account_id,stripe_session_id,amount_cents,currency,description,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').bind(sessionId,tenant,usage.client_id,usageId,conn.stripe_account_id,r.data.id,amount,currency,description,'created',ts,ts).run();
  ...
  return json({ok:true,checkout_url:r.data.url,session_id:r.data.id,charge_record_id:sessionId,amount_cents:amount,currency,merchant:'connected agency account',platform_application_fee_cents:0});
 }
