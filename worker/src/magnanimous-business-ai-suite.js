...
import {resolveBusinessAIClientAccess} from './agency-client-access.js';
...
export async function handleBusinessAISuite(request,env,ctx,downstream){const u=new URL(request.url);if(!u.pathname.startsWith('/api/business-ai'))return null;const user=await currentUser(request,env);if(!user)return json({detail:'Sign in required.'},401);await ensure(env);await claimLegacyJobs(env,user);const tenant=String(user.tenant_id),userId=String(user.id),registeredIds=BUSINESS_AI_SUITE.map(x=>x[0]),clientAccess=await resolveBusinessAIClientAccess(env,user,registeredIds),allowedIds=new Set(clientAccess.allowed_ids),canUseTool=id=>!clientAccess.enforced||allowedIds.has(String(id));
if(u.pathname==='/api/business-ai/tools'&&request.method==='GET'){const visible=BUSINESS_AI_SUITE.filter(([id])=>canUseTool(id)).map(([id,name,description,category])=>{const route=CAPABILITY_ROUTES[id],external=externalFor(id);return{id,name,description,category,...route,execution_mode:['hyper-images','logo-maker','music-generator'].includes(id)?'native-specialized':'magnanimous-step-execution',external_connections_required:external,outside_action_required:external.length>0}});return json({name:'Magnanimous Business AI Suite',brain:'Magnanimous AI',tools:visible,count:visible.length,registered_count:BUSINESS_AI_SUITE.length,client_access_enforced:Boolean(clientAccess.enforced),client:clientAccess.membership?{id:clientAccess.membership.client_id,name:clientAccess.membership.client_name,role:clientAccess.membership.role}:null,accessibility_standard:'keyboard-first, semantic labels, responsive layouts, plain-language errors, accessible generated-content metadata',truth_boundary:'External telecom, messaging, domain, storage, publishing or payment actions are only live when the corresponding authorized connection is actually ready.'})}
if(u.pathname==='/api/business-ai/jobs'&&request.method==='GET'){const{results=[]}=await env.DB.prepare('SELECT * FROM magnanimous_business_ai_jobs WHERE tenant_id=? AND user_id=? ORDER BY updated_at DESC LIMIT 100').bind(tenant,userId).all();const items=[];for(const row of results)if(canUseTool(row.tool_id))items.push(await jobView(env,user,row));return json({items,client_access_enforced:Boolean(clientAccess.enforced)})}
const jobMatch=u.pathname.match(/^\/api\/business-ai\/jobs\/([^/]+)(?:\/(verify|execute))?$/);
if(jobMatch&&request.method==='GET'){const row=await env.DB.prepare('SELECT * FROM magnanimous_business_ai_jobs WHERE id=? AND tenant_id=? AND user_id=?').bind(jobMatch[1],tenant,userId).first();if(!row)return json({detail:'Business AI job not found.'},404);if(!canUseTool(row.tool_id))return json({detail:'This Business AI tool is disabled for your assigned client.'},403);return json(await jobView(env,user,row))}
if(jobMatch&&jobMatch[2]==='execute'&&request.method==='POST'){const row=await env.DB.prepare('SELECT * FROM magnanimous_business_ai_jobs WHERE id=? AND tenant_id=? AND user_id=?').bind(jobMatch[1],tenant,userId).first();if(!row)return json({detail:'Business AI job not found.'},404);if(!canUseTool(row.tool_id))return json({detail:'This Business AI tool is disabled for your assigned client.'},403);return runDirectExecution(request,env,ctx,downstream,user,row)}
if(jobMatch&&jobMatch[2]==='verify'&&request.method==='POST'){const row=await env.DB.prepare('SELECT * FROM magnanimous_business_ai_jobs WHERE id=? AND tenant_id=? AND user_id=?').bind(jobMatch[1],tenant,userId).first();if(!row)return json({detail:'Business AI job not found.'},404);if(!canUseTool(row.tool_id))return json({detail:'This Business AI tool is disabled for your assigned client.'},403);const view=await jobView(env,user,row);const status=view.verification.status==='verified'?'completed':view.verification.workflow_complete?'waiting':'working';await env.DB.prepare('UPDATE magnanimous_business_ai_jobs SET status=?,updated_at=? WHERE id=? AND tenant_id=? AND user_id=?').bind(status,now(),jobMatch[1],tenant,userId).run();return json({...view,status,detail:view.verification.status==='verified'?'Verified complete.':view.verification.workflow_complete?'Workflow complete. An authorized external connection is still required before the outside action can be claimed complete.':'Execution is not complete yet.'})}
if(u.pathname==='/api/business-ai/jobs'&&request.method==='POST'){const b=await request.json().catch(()=>({})),tool=BUSINESS_AI_SUITE.find(x=>x[0]===String(b.tool_id||''));if(!tool)return json({detail:'Choose a valid tool.'},400);if(!canUseTool(tool[0]))return json({detail:'This Business AI tool is disabled for your assigned client.'},403);const id=crypto.randomUUID(),ts=now(),title=txt(b.title||tool[1],180);const plan=planFor(tool[0],b.input||{});const work=await createWork(env,user,{title,goal:plan.goal||tool[2],specialist_id:tool[0],metadata:{source:'business-ai',business_ai_job_id:id,tool_id:tool[0]}});if(!work)return json({detail:'Magnanimous Work Engine could not create this job.'},500);for(const step of plan.steps)await addWorkStep(env,user,work.id,{title:step.name,status:'planned'});const linked=await getWork(env,user,work.id);await env.DB.prepare('INSERT INTO magnanimous_business_ai_jobs(id,tenant_id,user_id,tool_id,title,input_json,output_json,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(id,tenant,userId,tool[0],title,JSON.stringify(b.input||{}),JSON.stringify({...plan,work_id:work.id}),'working',ts,ts).run();return json({ok:true,id,tool:{id:tool[0],name:tool[1]},status:'working',work:linked,execution_plan:{...plan,work_id:work.id}},201)}
