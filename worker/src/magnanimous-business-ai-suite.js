...
async function runDirectExecution(request,env,ctx,downstream,user,row){
 ...
 const prompt=`MAGNANIMOUS BUSINESS AI TASK
...
Return the actual useful draft/specification now. Clearly distinguish created content from outside actions that still require authorization or a live connection.`;
 const headers=new Headers(request.headers);headers.set('content-type','application/json');headers.delete('content-length');
 ...
 const work=view.work;if(work){const step=work.steps.find(x=>x.status==='pending'||x.status==='working');if(step)await updateWorkStep(env,user,work.id,step.id,{status:'completed',result:artifact});}
 const fresh=work?await getWork(env,user,work.id):null,out={...(view.output||{}),work_id:work?.id||view.output?.work_id,latest_artifact:artifact,last_execution_at:now(),direct_execution_tool:adapter.tool};
 await env.DB.prepare('UPDATE magnanimous_business_ai_jobs SET output_json=?,status=?,updated_at=? WHERE id=? AND tenant_id=?').bind(JSON.stringify(out),'working',now(),row.id,String(user.tenant_id)).run();
 return json({ok:true,id:row.id,artifact,work:fresh,verification:(await jobView(env,user,{...row,output_json:JSON.stringify(out),status:'working'})).verification,external_action_performed:false});
}
export async function handleBusinessAISuite(request,env,ctx,downstream){const u=new URL(request.url);if(!u.pathname.startsWith('/api/business-ai'))return null;const user=await currentUser(request,env);if(!user)return json({detail:'Sign in required.'},401);await ensure(env);const tenant=String(user.tenant_id);
...
if(jobMatch&&jobMatch[2]==='execute'&&request.method==='POST'){const row=await env.DB.prepare('SELECT * FROM magnanimous_business_ai_jobs WHERE id=? AND tenant_id=?').bind(jobMatch[1],tenant).first();if(!row)return json({detail:'Business AI job not found.'},404);return runDirectExecution(request,env,ctx,downstream,user,row)}
