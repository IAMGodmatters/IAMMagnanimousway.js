...
function publicArchitecture(){return {...MAGNANIMOUS_SOVEREIGN_STACK,execution_provider_details_private:true,products:PRODUCTS,skill_foundry:{policy:'learn portable capability patterns, workflows, schemas, evaluations and verified recipes; do not ingest proprietary hidden prompts, model weights, credentials or protected implementation',lifecycle:['observe','abstract','reimplement','test','verify','promote','monitor','improve'],adapter_count:ADAPTERS.length}};}

async function settings(env,user){await ensureSchema(env);const {results=[]}=await env.DB.prepare('SELECT product_id,enabled,white_label,brand_name,custom_domain,config_json,updated_at FROM sovereign_product_settings WHERE tenant_id=?').bind(user.tenant_id).all();return results.map(x=>({...x,enabled:!!x.enabled,white_label:!!x.white_label,config:(()=>{try{return JSON.parse(x.config_json||'{}')}catch{return{}}})()}));}

async function saveProduct(request,env,user){await ensureSchema(env);const body=await request.json().catch(()=>({}));const id=String(body.product_id||'').trim();if(!PRODUCTS.some(x=>x.id===id))return json({detail:'Unknown Magnanimous product.'},400);const ts=now(),brand=String(body.brand_name||'').trim().slice(0,120),domain=String(body.custom_domain||'').trim().slice(0,255),config=JSON.stringify(body.config&&typeof body.config==='object'?body.config:{}).slice(0,12000);await env.DB.prepare(`INSERT INTO sovereign_product_settings(tenant_id,product_id,enabled,white_label,brand_name,custom_domain,config_json,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(tenant_id,product_id) DO UPDATE SET enabled=excluded.enabled,white_label=excluded.white_label,brand_name=excluded.brand_name,custom_domain=excluded.custom_domain,config_json=excluded.config_json,updated_at=excluded.updated_at`).bind(user.tenant_id,id,body.enabled===false?0:1,body.white_label?1:0,brand,domain,config,ts).run();return json({ok:true,product_id:id,enabled:body.enabled!==false,white_label:!!body.white_label,brand_name:brand,custom_domain:domain});}

async function evaluateReasoning(request,env,user){await ensureSchema(env);const body=await request.json().catch(()=>({}));const score=Math.max(0,Math.min(1,Number(body.score||0))),verified=body.verified?1:0;await env.DB.prepare('INSERT INTO sovereign_reasoning_evaluations(tenant_id,user_id,task_class,score,verified,notes,created_at) VALUES(?,?,?,?,?,?,?)').bind(user.tenant_id,user.id,String(body.task_class||'general').slice(0,80),score,verified,String(body.notes||'').slice(0,2000),now()).run();return json({ok:true,score,verified:!!verified,learning_target:'Magnanimous reasoning policy and routing'});}

async function skills(env,user){await ensureSchema(env);const {results=[]}=await env.DB.prepare('SELECT skill_key,name,capability_class,source_kind,source_adapter,recipe_json,evidence_json,quality_score,verified,status,updated_at FROM magnanimous_skill_registry WHERE tenant_id=? ORDER BY verified DESC,quality_score DESC,updated_at DESC LIMIT 250').bind(user.tenant_id).all();return results.map(x=>({...x,verified:!!x.verified,recipe:(()=>{try{return JSON.parse(x.recipe_json)}catch{return{}}})(),evidence:(()=>{try{return JSON.parse(x.evidence_json)}catch{return{}}})()}));}

async function learnSkill(request,env,user){await ensureSchema(env);const b=await request.json().catch(()=>({}));const key=String(b.skill_key||'').trim().toLowerCase().replace(/[^a-z0-9._-]+/g,'-').slice(0,120);const name=String(b.name||'').trim().slice(0,160);if(!key||!name)return json({detail:'skill_key and name are required.'},400);const source=String(b.source_adapter||'').trim().slice(0,80);const recipe=b.recipe&&typeof b.recipe==='object'?b.recipe:{};const evidence=b.evidence&&typeof b.evidence==='object'?b.evidence:{};const forbidden=JSON.stringify({recipe,evidence}).match(/api[_ -]?key|password|secret[_ -]?key|private[_ -]?prompt|model[_ -]?weights/i);if(forbidden)return json({detail:'Protected credentials, hidden prompts, secrets, or model-weight material cannot be assimilated.'},400);const score=Math.max(0,Math.min(1,Number(b.quality_score||0)));const verified=!!b.verified&&score>=0.8;const status=verified?'verified':'candidate';const ts=now();await env.DB.prepare(`INSERT INTO magnanimous_skill_registry(tenant_id,skill_key,name,capability_class,source_kind,source_adapter,recipe_json,evidence_json,quality_score,verified,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(tenant_id,skill_key) DO UPDATE SET name=excluded.name,capability_class=excluded.capability_class,source_kind=excluded.source_kind,source_adapter=excluded.source_adapter,recipe_json=excluded.recipe_json,evidence_json=excluded.evidence_json,quality_score=excluded.quality_score,verified=excluded.verified,status=excluded.status,updated_at=excluded.updated_at`).bind(user.tenant_id,key,name,String(b.capability_class||'general').slice(0,80),String(b.source_kind||'observed-workflow').slice(0,80),source,JSON.stringify(recipe).slice(0,16000),JSON.stringify(evidence).slice(0,16000),score,verified?1:0,status,ts,ts).run();return json({ok:true,skill_key:key,status,verified,principle:'Magnanimous learned a portable recipe/pattern, not proprietary provider internals.'});}

export async function handleMagnanimousSovereign(request,env){const url=new URL(request.url),path=url.pathname;if(!path.startsWith('/api/magnanimous/sovereign'))return null;if(path==='/api/magnanimous/sovereign/architecture'&&request.method==='GET')return json(publicArchitecture());const user=await currentUser(request,env);if(!user)return json({detail:'Sign in required.'},401);if(path==='/api/magnanimous/sovereign/products'&&request.method==='GET')return json({products:PRODUCTS,settings:await settings(env,user),provider_details_private:true});if(path==='/api/magnanimous/sovereign/products'&&request.method==='POST')return saveProduct(request,env,user);if(path==='/api/magnanimous/sovereign/reasoning/evaluate'&&request.method==='POST')return evaluateReasoning(request,env,user);if(path==='/api/magnanimous/sovereign/adapters'&&request.method==='GET')return json({adapters:ADAPTERS.map(({id,class:capability_class,role,native_target,portable})=>({id,capability_class,role,native_target,portable})),rule:'Adapters are replaceable and never own Magnanimous identity or memory.'});if(path==='/api/magnanimous/sovereign/skills'&&request.method==='GET')return json({skills:await skills(env,user)});if(path==='/api/magnanimous/sovereign/skills/learn'&&request.method==='POST')return learnSkill(request,env,user);return json({detail:'Sovereign capability route not found.'},404);}
