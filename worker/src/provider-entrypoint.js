...
function standaloneRuntime(env){return String(env?.MAGNANIMOUS_RUNTIME||'').trim()==='standalone-node'}
async function standaloneEdgeComputeFallback(request,env,prompt){
  if(!standaloneRuntime(env)||request.headers.get('x-magnanimous-edge-fallback')==='1')return null;
  ...
  try{
    ...
    if(current.host.toLowerCase()===target.host.toLowerCase())return null;
  }catch{return null}
  const safePrompt=String(prompt||'').replaceAll(MEMORY_MARKER,'\n\nCENTRAL BRAIN CONTEXT');
  ...
  if(!response.ok)throw new Error(String(data?.detail||data?.error||`free edge compute returned HTTP ${response.status}`));
  const text=String(data?.output||data?.answer||'').trim();
  if(!text)throw new Error('free edge compute returned no text');
  return{text,model:String(data?.model||'edge-free'),provider:'magnanimous-edge-free'};
}
...
async function handle(request, env) {
  ...
  if (url.pathname === '/api/chat' && request.method === 'POST') {
    ...
    if (!candidates.length) {
      if(requested==='auto'){
        try{
          const edge=await standaloneEdgeComputeFallback(request,env,groundedMessage);
          if(edge)return json({output:edge.text,provider:edge.provider,provider_name:'Magnanimous free edge compute',model:edge.model,magnanimous:true,operator:true,command_role:'commander-in-chief',provider_role:'replaceable-execution-engine',routed_automatically:true,route_task:task,native_capability:capability,route_policy:'free-first',fallback_candidates:[],standalone_edge_fallback:true,grounded:grounding.sources.length>0,sources:grounding.sources,link_learning:{enabled:body.learn_links!==false,absorbed:absorbedLinks.length,results:linkLearning}});
        }catch(error){console.error('Magnanimous standalone free edge fallback failed',String(error?.message||error))}
      }
    ...
    if(requested==='auto'){
      try{
        const edge=await standaloneEdgeComputeFallback(request,env,groundedMessage);
        if(edge)return json({output:edge.text,provider:edge.provider,provider_name:'Magnanimous free edge compute',model:edge.model,magnanimous:true,operator:true,command_role:'commander-in-chief',provider_role:'replaceable-execution-engine',routed_automatically:true,route_task:task,native_capability:capability,route_policy:'free-first',fallback_candidates:candidates.map(x=>x.id),standalone_edge_fallback:true,grounded:grounding.sources.length>0,sources:grounding.sources,link_learning:{enabled:body.learn_links!==false,absorbed:absorbedLinks.length,results:linkLearning}});
      }catch(error){errors.push(`Magnanimous free edge compute: ${error?.message||'fallback failed'}`)}
    }
