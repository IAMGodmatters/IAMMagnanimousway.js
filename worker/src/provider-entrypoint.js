...
async function handle(request, env) {
  ...
  if (url.pathname === '/api/providers' && request.method === 'GET') {
    return json({ free_first: true, metered_providers_enabled: meteredEnabled(env), providers: PROVIDERS.map(p => ({ id: p.id, name: p.name, configured: configured(env, p), enabled: p.tier !== 'metered' || meteredEnabled(env), tier: p.tier, type: 'ai' })) });
  }
  if (url.pathname === '/api/chat' && request.method === 'POST') {
    ...
    for (const p of configuredCandidates) {
      try { const output = await callProvider(p.id, env, message, body.model); return json({ output, provider: p.id, provider_name: p.name }); }
