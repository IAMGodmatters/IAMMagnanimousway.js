...
export const ADVANCED_CRM_CAPABILITIES=[
 ...
 {id:'predictive-model-lab',name:'Tenant-specific conversion, opportunity-win, churn and forecast dataset and evaluation lab'},
...
const CRM_ML_TYPES=['conversion','opportunity_win','churn','forecast'];
async function buildMlDataset(env,t,modelType){
 if(!CRM_ML_TYPES.includes(modelType))throw new Error('unsupported_model_type');
 const readiness=await predictiveReadiness(env,t);
 if(!readiness.ready)return{blocked:true,code:'predictive_readiness_required',readiness};
 const rows=await env.DB.prepare(`SELECT o.id,o.contact_id,o.value,o.probability,o.created_at,o.updated_at,lower(COALESCE(ps.kind,o.stage)) outcome,(SELECT COUNT(*) FROM crm_activities a WHERE a.tenant_id=o.tenant_id AND a.contact_id=o.contact_id AND a.created_at<=o.updated_at) activity_count,(SELECT COUNT(*) FROM crm_cases c WHERE c.tenant_id=o.tenant_id AND c.contact_id=o.contact_id AND c.created_at<=o.updated_at) case_count FROM crm_opportunities o LEFT JOIN crm_pipeline_stages ps ON ps.id=o.stage_id AND ps.tenant_id=o.tenant_id WHERE o.tenant_id=? AND lower(COALESCE(ps.kind,o.stage)) IN ('won','lost','closed') ORDER BY o.updated_at,o.id`).bind(t).all();
 const data=(rows.results||[]).map(r=>({opportunity_id:r.id,features:{value:Number(r.value||0),prior_probability:Number(r.probability||0),age_days:Math.max(0,Math.floor((Number(r.updated_at||0)-Number(r.created_at||0))/86400)),activity_count:Number(r.activity_count||0),case_count:Number(r.case_count||0)},label:String(r.outcome)==='won'?1:0}));
 const positives=data.filter(x=>x.label===1).length,id=uid(),ts=now(),features=['value','prior_probability','age_days','activity_count','case_count'];
 await env.DB.prepare('INSERT INTO crm_ml_datasets(id,tenant_id,model_type,schema_version,row_count,positive_count,negative_count,feature_names_json,dataset_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(id,t,modelType,'v1',data.length,positives,data.length-positives,JSON.stringify(features),JSON.stringify(data).slice(0,900000),ts).run();
 return{blocked:false,id,model_type:modelType,row_count:data.length,positive_count:positives,negative_count:data.length-positives,feature_names:features,readiness,provenance:'tenant-specific genuine closed opportunity outcomes; no synthetic labels'};
}
...
export async function handleAdvancedCrm(request,env,user,body={}){
 ...
 if(url.pathname==='/api/operations/crm/ml/datasets/build'&&request.method==='POST'){try{const result=await buildMlDataset(env,t,txt(body.model_type||'opportunity_win',40));if(result.blocked)return json(result,409);await audit(env,user,'crm_ml_dataset_built',{dataset_id:result.id,model_type:result.model_type,row_count:result.row_count});return json(result,201)}catch(e){return json({detail:String(e?.message||e)},400)}}
 if(url.pathname==='/api/operations/crm/ml/experiments'){
  if(request.method==='GET'){const r=await env.DB.prepare('SELECT id,dataset_id,model_type,status,algorithm,train_rows,validation_rows,metrics_json,created_at,updated_at FROM crm_ml_experiments WHERE tenant_id=? ORDER BY created_at DESC LIMIT 200').bind(t).all();return json({items:(r.results||[]).map(x=>({...x,metrics:parse(x.metrics_json,{})}))})}
  if(request.method==='POST'){const readiness=await predictiveReadiness(env,t);if(!readiness.ready)return json({code:'predictive_readiness_required',readiness},409);const dataset=await owned(env,t,'crm_ml_datasets',txt(body.dataset_id,80));if(!dataset)return json({detail:'Dataset not found.'},404);const n=Number(dataset.row_count||0),validation=Math.max(1,Math.floor(n*.2)),id=uid(),ts=now();await env.DB.prepare('INSERT INTO crm_ml_experiments(id,tenant_id,dataset_id,model_type,status,algorithm,train_rows,validation_rows,metrics_json,artifact_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,t,dataset.id,dataset.model_type,'prepared','logistic_regression_candidate',n-validation,validation,JSON.stringify({evaluation_required:true,minimum_metrics:['roc_auc','precision','recall','brier_score'],promotion_rule:'must beat heuristic baseline on held-out tenant data'}),JSON.stringify({trained:false,note:'Training is intentionally not claimed until an approved training runtime evaluates this tenant-specific dataset.'}),ts,ts).run();await audit(env,user,'crm_ml_experiment_prepared',{experiment_id:id,dataset_id:dataset.id});return json({id,status:'prepared',trained:false,model_type:dataset.model_type,train_rows:n-validation,validation_rows:validation},201)}
 }
