...
async function checkout(request,env,user,body){
 ...
 const origin=siteOrigin(request,env);form.set('success_url',`${origin}/pricing?checkout=success&plan=${encodeURIComponent(plan)}&session_id={CHECKOUT_SESSION_ID}`);form.set('cancel_url',`${origin}/pricing?checkout=cancelled&plan=${encodeURIComponent(plan)}`);
