...
async function ensure(env){
 ...
 // Keep website-wide public reviews separate from the older tenant-scoped billing reviews table.
 // This allows visitors to leave moderated site feedback without changing legacy subscription data.
 await env.DB.prepare(`CREATE TABLE IF NOT EXISTS site_public_reviews (
...
async function publicReviewSummary(env){
 const stats=await env.DB.prepare("SELECT COUNT(*) total,AVG(rating) average_rating FROM site_public_reviews WHERE status='approved'").first();
...
export async function handleSupportFeedback(request,env,{platformOwner=false}={}){
 ...
 try{
  ...
  if(url.pathname==='/api/support/reviews'&&request.method==='GET'){
   const {results=[]}=await env.DB.prepare("SELECT id,reviewer_name,title,message,rating,owner_response,verified_user,created_at FROM site_public_reviews WHERE status='approved' ORDER BY created_at DESC LIMIT 100").all();
  ...
  if(url.pathname==='/api/support/reviews'&&request.method==='POST'){
   ...
   await env.DB.prepare('INSERT INTO site_public_reviews(id,reviewer_name,title,message,rating,status,owner_response,user_id,verified_user,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(id,name||'Platform user',title,message,rating,'pending','',user?String(user.id):'',user?1:0,ts,ts).run();
  ...
  if(url.pathname.startsWith('/api/support/owner/reviews')){
   ...
   if(url.pathname==='/api/support/owner/reviews'&&request.method==='GET'){
    const filter=String(url.searchParams.get('status')||'');let sql='SELECT * FROM site_public_reviews',args=[];
    ...
    const counts=await env.DB.prepare("SELECT COUNT(*) total,SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) pending_count,SUM(CASE WHEN status='approved' THEN 1 ELSE 0 END) approved_count FROM site_public_reviews").first();
   ...
   if(rm&&request.method==='PUT'){
    const b=await request.json().catch(()=>({})),old=await env.DB.prepare('SELECT * FROM site_public_reviews WHERE id=?').bind(rm[1]).first();if(!old)return json({error:'Review not found.'},404);
    ...
    await env.DB.prepare('UPDATE site_public_reviews SET status=?,owner_response=?,updated_at=? WHERE id=?').bind(status,response,now(),rm[1]).run();
    return json({ok:true,review:await env.DB.prepare('SELECT * FROM site_public_reviews WHERE id=?').bind(rm[1]).first()});
   }
  }
