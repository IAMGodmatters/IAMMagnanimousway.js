...
const TRIGGERS=new Set(['booking.created','funnel.created','review.created','usage.created','inbox.received','manual']);
const ACTIONS=new Set(['create-work','create-inbox']);
const RULE_STATUSES=new Set(['active','paused']);
...
export async function handleAgencyAutomations(request,env){
 ...
 try{
  ...
  if(url.pathname==='/api/agency/automations'){
   ...
   if(request.method==='POST'){if(!owner)return json({detail:'Workspace owner access required to create automations.'},403);const b=await request.json().catch(()=>({})),client=text(b.client_id,80)||null;if(client&&!await clientExists(env,tenant,client))return json({detail:'Choose a valid client account.'},400);const name=text(b.name,180),trigger=text(b.trigger_type,80),action=text(b.action_type,80),status=String(b.status||'active');if(!name||!trigger||!action)return json({detail:'Name, trigger and action are required.'},400);if(!TRIGGERS.has(trigger))return json({detail:'Unsupported automation trigger.'},400);if(!ACTIONS.has(action))return json({detail:'Unsupported automation action.'},400);if(!RULE_STATUSES.has(status))return json({detail:'Automation status must be active or paused.'},400);const id=crypto.randomUUID(),ts=now();await env.DB.prepare('INSERT INTO agency_automations(id,tenant_id,client_id,name,trigger_type,action_type,action_config_json,status,run_count,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(id,tenant,client,name,trigger,action,JSON.stringify(b.action_config||{}).slice(0,12000),status,0,ts,ts).run();return json({ok:true,id},201)}
  ...
  if(m&&request.method==='PATCH'){if(!owner)return json({detail:'Workspace owner access required.'},403);const rule=await env.DB.prepare('SELECT * FROM agency_automations WHERE id=? AND tenant_id=?').bind(m[1],tenant).first();if(!rule)return json({detail:'Automation not found.'},404);const b=await request.json().catch(()=>({})),trigger=text(b.trigger_type===undefined?rule.trigger_type:b.trigger_type,80),action=text(b.action_type===undefined?rule.action_type:b.action_type,80),status=String(b.status===undefined?rule.status:b.status);if(!TRIGGERS.has(trigger))return json({detail:'Unsupported automation trigger.'},400);if(!ACTIONS.has(action))return json({detail:'Unsupported automation action.'},400);if(!RULE_STATUSES.has(status))return json({detail:'Automation status must be active or paused.'},400);await env.DB.prepare('UPDATE agency_automations SET name=?,trigger_type=?,action_type=?,action_config_json=?,status=?,updated_at=? WHERE id=? AND tenant_id=?').bind(text(b.name===undefined?rule.name:b.name,180),trigger,action,JSON.stringify(b.action_config===undefined?config(rule.action_config_json):b.action_config).slice(0,12000),status,now(),m[1],tenant).run();return json({ok:true,status})}
