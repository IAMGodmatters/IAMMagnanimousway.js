...
export async function handleWhiteLabelOS(request,env){const u=new URL(request.url);if(!u.pathname.startsWith('/api/white-label-os'))return null;const user=await currentUser(request,env);if(!user)return json({detail:'Sign in required.'},401);await ensure(env);const tenant=String(user.tenant_id);
...
const m=u.pathname.match(/^\/api\/white-label-os\/(catalog|projects|contracts|learning|affiliates)$/);if(!m)return json({detail:'White Label OS endpoint not found.'},404);const [table,fields]=specs[m[1]];
...
if(request.method==='POST'){if(!owner(user))return json({detail:'Owner or admin access required.'},403);const b=await request.json().catch(()=>({})),id=crypto.randomUUID(),ts=now(),vals=fields.map(f=>f.includes('price')||f.includes('percent')||f==='due_at'?Number(b[f]||0):txt(b[f],f==='body'?12000:4000));if(!txt(b.name||b.title,180))return json({detail:'A name or title is required.'},400);await env.DB.prepare(`INSERT INTO ${table}(id,tenant_id,${fields.join(',')},created_at,updated_at) VALUES(?,?${',?'.repeat(fields.length)},?,?)`).bind(id,tenant,...vals,ts,ts).run();return json({ok:true,id,module:m[1]},201)}
