...
function clientAppProvider(context,id){
 ...
 if(id==='whatsapp')return{id:'magnanimous-whatsapp-inbox',name:'Magnanimous Product Inbox',mode:'hybrid',status:'native-ready',ready:true,connected:true,setup_path:'/white-label/whatsapp',external_transport:{id:'meta-whatsapp',name:'Meta WhatsApp Business Platform',required_for_delivery:true,status:context.whatsappConnected?'connected':'setup-required',ready:context.whatsappConnected,connected:context.whatsappConnected,setup_path:'/white-label/whatsapp'}};
...
if(u.pathname==='/api/white-label-os/client-apps'){
 ...
 if(request.method==='GET'){
  ...
  return json({client,apps,available_apps:apps.map(({app_id,name,group,href,provider})=>({app_id,name,group,href,provider})),catalog_count:CLIENT_APP_CATALOG.length,provider_policy:'Every client app has a registered route and an explicit Magnanimous execution provider. External-network transports are reported separately and remain setup-required until a verified account route exists.',authorization_boundary:'Client app selections control menu visibility and packaging. They do not become a security authorization boundary until the signed-in end user is reliably mapped to this client account.'});
 }
