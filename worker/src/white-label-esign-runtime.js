...
function view(row){return{id:row.id,client_id:row.client_id,contract_id:row.contract_id,signer_name:row.signer_name,signer_email:row.signer_email,status:row.status,document_title:row.document_title,document_hash:row.document_hash,sent_at:row.sent_at,viewed_at:row.viewed_at,signed_at:row.signed_at,created_at:row.created_at,updated_at:row.updated_at,audit:{consent_text:row.consent_text,signature_text:row.signature_text||'',signer_user_agent:row.signer_user_agent||'',signer_ip_hash:row.signer_ip_hash||''}}}
async function createTokenRecord(env,tenant,user,b,existing=null){
 const c=await contract(env,tenant,clean(b.contract_id||existing?.contract_id,100));if(!c)return{error:json({detail:'Choose a valid proposal/agreement in this workspace.'},404)};
 ...
 const raw=token(),tokenHash=await sha(raw),snapshotTitle=clean(c.title,500),snapshotBody=clean(c.body,12000),documentHash=await sha(`${snapshotTitle}\n\n${snapshotBody}`),ts=now(),id=existing?.id||crypto.randomUUID();
 if(existing){
  await env.DB.prepare("UPDATE agency_esign_requests SET signer_name=?,signer_email=?,token_hash=?,status='pending',consent_text=?,document_title=?,document_body=?,document_hash=?,signature_text='',signer_user_agent='',signer_ip_hash='',sent_at=NULL,viewed_at=NULL,signed_at=NULL,updated_at=? WHERE id=? AND tenant_id=?")
   .bind(signerName,signerEmail,tokenHash,CONSENT,snapshotTitle,snapshotBody,documentHash,ts,id,tenant).run();
 }else{
  await env.DB.prepare(`INSERT INTO agency_esign_requests(id,tenant_id,client_id,contract_id,signer_name,signer_email,token_hash,status,consent_text,document_title,document_body,document_hash,created_at,updated_at)
  VALUES(?,?,?,?,?,?,?,'pending',?,?,?,?,?,?)`).bind(id,tenant,clean(c.client_id,100),c.id,signerName,signerEmail,tokenHash,CONSENT,snapshotTitle,snapshotBody,documentHash,ts,ts).run();
 }
...
async function publicRow(env,raw){
 const hash=await sha(raw);return env.DB.prepare('SELECT * FROM agency_esign_requests WHERE token_hash=? LIMIT 1').bind(hash).first();
}
export async function handlePublicEsign(request,env){
 ...
 m=url.pathname.match(/^\/sign\/([^/]+)\/accept$/);if(m&&request.method==='POST'){
  ...
  const ua=clean(request.headers.get('user-agent'),500),network=clean(request.headers.get('cf-connecting-ip')||request.headers.get('x-forwarded-for'),200),ipHash=network?await sha(`${row.id}:${network}`):'',ts=now();
