...
export default function ContactCenter(){
  ...
  const [supervisionConsent,setSupervisionConsent]=useState(false);
  const [supervisionNotice,setSupervisionNotice]=useState(false);
  const [recordingJurisdiction,setRecordingJurisdiction]=useState('');
  const [activeSupervision,setActiveSupervision]=useState<Record<string,string>>({});
  ...
  async function dialReserved(){
    ...
    try{
      ...
      setNotice(`Call ${result.id||result.call_id||''} queued through the Magnanimous carrier route. Save the disposition when the interaction ends.`);
  ...
  async function startSupervision(callId:number,mode:'monitor'|'whisper'|'barge'){
    if(!supervisionConsent||!supervisionNotice){setError('Confirm participant consent and supervision notice before starting supervisor audio.');return}
    setBusy(`supervise-${callId}`);setError('');
    try{const result=await send(`/api/contact-center/supervisor/calls/${callId}/session`,{mode,consent_confirmed:true,notice_confirmed:true});setActiveSupervision(x=>({...x,[String(callId)]:result.id}));setNotice(`${mode} session started for call #${callId}.`);await load(token,false)}catch(e:any){setError(e.message)}finally{setBusy('')}
  }
  async function stopSupervision(callId:number){
    const id=activeSupervision[String(callId)];if(!id)return;
    setBusy(`supervise-${callId}`);setError('');
    try{await send(`/api/contact-center/supervisor/sessions/${id}`,{},'DELETE');setActiveSupervision(x=>{const n={...x};delete n[String(callId)];return n});setNotice(`Supervisor audio stopped for call #${callId}.`);await load(token,false)}catch(e:any){setError(e.message)}finally{setBusy('')}
  }
  async function startRecording(callId:number){
    if(!supervisionConsent||!supervisionNotice||recordingJurisdiction.trim().length<2){setError('Confirm consent, recording notice, and jurisdiction before recording.');return}
    setBusy(`record-${callId}`);setError('');
    try{const result=await send(`/api/contact-center/calls/${callId}/recording`,{consent_confirmed:true,notice_confirmed:true,jurisdiction:recordingJurisdiction.trim()});setActiveRecording(x=>({...x,[String(callId)]:result.id}));setNotice(`Consent-gated recording started for call #${callId}.`);await load(token,false)}catch(e:any){setError(e.message)}finally{setBusy('')}
  }
  async function stopRecording(callId:number){
    setBusy(`record-${callId}`);setError('');
    try{await send(`/api/contact-center/calls/${callId}/recording`,{},'DELETE');setActiveRecording(x=>{const n={...x};delete n[String(callId)];return n});setNotice(`Recording stopped for call #${callId}.`);await load(token,false)}catch(e:any){setError(e.message)}finally{setBusy('')}
  }

  const tabs=[['overview','Command'],['dialer','Dialer'],['ivr','IVR'],['inbox','Inbox'],['supervisor','Supervisor'],['quality','QA + WFM']];

  return <main className="cx-root">
    <header className="cx-head"><div><a href="/">← I AM Platform</a><small>I AM MAGNANIMOUS WAY™ · PROFESSIONAL CONTACT CENTER</small><h1>Magnanimous CX Command</h1><p>One workspace for browser and carrier calling, scripts, rebuttals, campaigns, IVR, AI reception, callbacks, voicemail, workforce, quality, analytics and digital service—built to prefer the cheapest valid calling route.</p></div><div className="cx-live"><i/>{busy==='load'?'SYNCING':'LIVE'}</div></header>
    ...
    {tab==='overview'&&<>
      ...
      <section className="cx-grid3">
        <Panel title="Agent desk"><Feature>Live opening, discovery, value and closing scripts</Feature><Feature>Searchable one-click rebuttals and coaching rules</Feature><Feature>Free browser calling plus cost-first carrier fallback</Feature><a className="cx-link" href="/agent-desk">Open Agent Desk →</a><a className="cx-link second" href="/softphone">Carrier Phone →</a></Panel>
    ...
    {tab==='supervisor'&&<>
      <section className="cx-stats"><Stat label="Live calls" value={(supervisor.calls||[]).length} sub="floor view"/><Stat label="Agents" value={(supervisor.agents||[]).length} sub={`${(supervisor.agents||[]).filter((x:Row)=>x.status==='available').length} available`}/><Stat label="Coach rules" value={(supervisor.assist_rules||[]).length} sub="live assist"/><Stat label="Monitor" value={supervisor.supervisor_audio?.monitor?'READY':'GATED'} sub="consent required"/><Stat label="Whisper / barge" value={supervisor.supervisor_audio?.whisper&&supervisor.supervisor_audio?.barge?'READY':'GATED'} sub="native core only"/><Stat label="Recording" value={supervisor.supervisor_audio?.bridge_recording?'READY':'GATED'} sub="notice + jurisdiction"/></section>
      <section className="cx-two">
        <Panel title="Consent-gated supervisor controls">
          <label className="cx-check"><input type="checkbox" checked={supervisionConsent} onChange={e=>setSupervisionConsent(e.target.checked)}/><span>I confirm the participants have given the required consent for this supervision/recording action.</span></label>
          <label className="cx-check"><input type="checkbox" checked={supervisionNotice} onChange={e=>setSupervisionNotice(e.target.checked)}/><span>I confirm the required supervision/recording notice has been given.</span></label>
          <label>Recording jurisdiction<input value={recordingJurisdiction} onChange={e=>setRecordingJurisdiction(e.target.value)} placeholder="Example: US-CA or PH"/></label>
          <p className="muted">Controls stay disabled unless the protected Magnanimous Telecom Core reports the private Stasis event stream ready. No covert monitoring is enabled.</p>
          <table><thead><tr><th>Call</th><th>Agent</th><th>Status</th><th>Party</th><th>Supervisor audio</th><th>Recording</th></tr></thead><tbody>{(supervisor.calls||[]).map((call:Row)=>{const key=String(call.id),active=activeSupervision[key],recording=activeRecording[key],audio=supervisor.supervisor_audio||{};return <tr key={call.id}><td>#{call.id}</td><td>{call.agent_name||'AI / unassigned'}</td><td>{call.status}</td><td>{call.direction==='inbound'?call.caller:call.callee}</td><td>{active?<button onClick={()=>stopSupervision(Number(call.id))} disabled={busy===`supervise-${call.id}`}>Stop</button>:<div className="cx-actions"><button onClick={()=>startSupervision(Number(call.id),'monitor')} disabled={!audio.monitor||busy===`supervise-${call.id}`}>Monitor</button><button onClick={()=>startSupervision(Number(call.id),'whisper')} disabled={!audio.whisper||busy===`supervise-${call.id}`}>Whisper</button><button onClick={()=>startSupervision(Number(call.id),'barge')} disabled={!audio.barge||busy===`supervise-${call.id}`}>Barge</button></div>}</td><td>{recording?<button onClick={()=>stopRecording(Number(call.id))} disabled={busy===`record-${call.id}`}>Stop recording</button>:<button onClick={()=>startRecording(Number(call.id))} disabled={!audio.bridge_recording||busy===`record-${call.id}`}>Record</button>}</td></tr>})}</tbody></table>
          {!(supervisor.calls||[]).length&&<Empty text="No live calls."/>}
        </Panel>
        <Panel title="Live AI coach"><input value={assist.name} onChange={e=>setAssist({...assist,name:e.target.value})} placeholder="Rule name"/><input value={assist.trigger_phrase} onChange={e=>setAssist({...assist,trigger_phrase:e.target.value})} placeholder="Trigger phrase, e.g. refund"/><textarea value={assist.guidance} onChange={e=>setAssist({...assist,guidance:e.target.value})} placeholder="Guidance shown to the agent in the moment"/><button className="primary" onClick={addAssist} disabled={busy==='assist'}>Add coaching rule</button>{(supervisor.assist_rules||[]).map((rule:Row)=><Item key={rule.id} title={rule.name} sub={`When “${rule.trigger_phrase}” → ${rule.guidance}`}/>)}</Panel>
      </section>
    </>}
    ...
    <footer className="cx-footer"><a href="/agent-desk">Agent Desk</a><a href="/softphone">Carrier Softphone</a><a href="/phone">Free WebRTC Phone</a><a href="/ai-receptionist">AI Receptionist</a><a href="/call-center-health">QA / WFM</a><a href="/crm">CRM</a><a href="/connections">Integrations</a><a href="/pricing">Plans</a></footer>
    <style jsx global>{`
