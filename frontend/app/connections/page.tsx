...
export default function Connections(){
  const[items,setItems]=useState<Integration[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(''),[shop,setShop]=useState(''),[telegram,setTelegram]=useState(''),[category,setCategory]=useState(''),[focusProvider,setFocusProvider]=useState('');
  ...
  useEffect(()=>{
    ...
    const q=new URLSearchParams(location.search),cat=q.get('category')||'',fp=q.get('provider')||'',connected=q.get('integration')||'';
    setCategory(cat);setFocusProvider(fp);
    if(window.opener&&connected){
      try{window.opener.postMessage({type:'iam-integration-connected',provider:connected,count:Number(q.get('connected')||0)},location.origin)}catch{}
      setTimeout(()=>window.close(),350);
      return;
    }
    const onMessage=(event:MessageEvent)=>{if(event.origin!==location.origin||event.data?.type!=='iam-integration-connected')return;setNotice(`${event.data.provider||'Account'} connected securely. No platform-owner approval was required.`);setBusy('');load()};
    window.addEventListener('message',onMessage);load();
    return()=>window.removeEventListener('message',onMessage);
  },[]);
  useEffect(()=>{if(!loading&&focusProvider)setTimeout(()=>document.getElementById(`provider-${focusProvider}`)?.scrollIntoView({behavior:'smooth',block:'center'}),80)},[loading,focusProvider]);
  const total=useMemo(()=>items.reduce((n,x)=>n+x.connected.length,0),[items]);
  const visible=useMemo(()=>{let list=category?items.filter(x=>x.category===category):items;if(focusProvider)list=[...list].sort((a,b)=>a.id===focusProvider?-1:b.id===focusProvider?1:0);return list},[items,category,focusProvider]);
  ...
  async function connect(item:Integration){
    ...
    try{
      ...
      setNotice(`${item.name} authorization is opening securely. I AM stays open and will refresh when the provider finishes.`);
      const popup=window.open(d.authorization_url,`iam-${item.id}-authorization`,'popup=yes,width=1080,height=780');
      if(!popup){location.href=d.authorization_url;return}
      const timer=window.setInterval(()=>{if(popup.closed){window.clearInterval(timer);setBusy('');load()}},900);
  ...
  return <main className="page">
    <header><a href="/">← Dashboard</a><span>{category==='email'?'EMAIL CONNECTIONS':'SELF-SERVICE CONNECTIONS'}</span></header>
    ...
    {category==='email'&&<section className="emailCard"><div><small>EMAIL-FIRST WORKFLOW</small><h2>Need the professional mailbox before connecting it?</h2><p>Use the I AM Business Email Center to choose an official provider, keep your setup checklist inside I AM, and verify MX/SPF/DMARC records before connecting Gmail or Outlook.</p></div><a href="/business-email">OPEN BUSINESS EMAIL NEXUS →</a></section>}

    <section className="grid">
      {loading?<div className="loading">Loading secure connection options…</div>:visible.map(item=>{const connected=item.connected.length>0;return <article id={`provider-${item.id}`} key={item.id} className={`${connected?'on ':''}${focusProvider===item.id?'focus':''}`}>
