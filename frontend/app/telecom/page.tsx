...
export default function TelecomPage(){
 ...
 async function askMagnanimous(event:FormEvent){
  ...
  const context=`You are Magnanimous AI operating inside Magnanimous Telecom. Magnanimous AI is the brain and Magnanimous Telecom is the public telecom service provider. Treat outside carriers, mobile networks, MVNOs, eSIM providers, SIP/media servers, mobile-core projects, charging systems, observability systems, model providers and infrastructure vendors only as replaceable tools beneath Magnanimous. Never offer, recommend, configure or expose Magnanimous Telecom as a white-label telecom product, telecom reseller platform, sub-carrier platform, customer-owned carrier, alternate provider brand, or backend that lets a customer start their own phone company. Customers may subscribe to Magnanimous Telecom SIM/eSIM lines, phone numbers, minutes, messaging, data, call-center lines and service bundles. Use carrier-grade separation of product catalog, service orders, subscriber lifecycle, normalized usage, rating/charging, balances, routing, fraud, observability, trouble tickets, roaming/settlement and network functions. Support prepaid, postpaid, hybrid, recurring, no-expiry, roaming, add-on, shared-balance, convertible-allowance and unlimited-policy concepts when appropriate. Treat EPC/5GC/IMS technology as an isolated lab/future operator path until lawful spectrum/RAN/interconnection and carrier authorization are actually verified. Never claim emergency calling, direct numbering authority, carrier authorization, a public telephone number, or mobile provisioning unless the live system indicates it is configured and verified. Never request or store raw SIM authentication credentials such as Ki, OPc or ADM keys. Help with telecom operations, architecture, customer service, routing, service assurance, compliance planning, fraud controls, retail service-plan design, charging, SIM/eSIM lifecycle, number lifecycle and troubleshooting. User request: ${prompt}`;
  try{
 ...
 const tools=[
  ...
  {href:'/telecom/operations',icon:'⚙',title:'Carrier Operations',text:'OSS/BSS, service orders, charging, balances, routing, fraud and assurance.'},
 ...
 return <main className={styles.shell}>
  <header className={styles.hero}>
   <nav><a href='/'>I AM MAGNANIMOUS WAY™</a><span>•</span><a href='/phone'>PHONE</a><span>•</span><a href='/telecom/plans'>PLANS</a><span>•</span><a href='/telecom/operations'>OPERATIONS</a><span>•</span><a href='/telecom/sim'>SIM + eSIM</a><span>•</span><a href='/telecom/network'>CARRIER ACCESS</a><span>•</span><a href='/telecom-standalone'>STANDALONE</a></nav>
   <div className={styles.heroGrid}>
    <div>
     ...
     <div className={styles.actions}><a className={styles.primary} href='/telecom/operations'>OPEN CARRIER OPERATIONS</a><a className={styles.secondary} href='/telecom/plans'>OPEN PLANS</a><a className={styles.secondary} href='/phone'>OPEN LIVE PHONE</a><a className={styles.secondary} href='/telecom/network'>OPEN CARRIER ACCESS</a></div>
    </div>
  ...
  <section className={styles.section}>
   ...
   <div className={styles.aiPanel}>
    <form onSubmit={askMagnanimous}>
     <textarea value={question} onChange={event=>setQuestion(event.target.value)} placeholder='Ask Magnanimous to plan routing, troubleshoot calls, manage service orders, design prepaid/postpaid plans, review charging or fraud, manage SIM/eSIM workflows, explain a compliance gate, or prepare the next carrier build step.'/>
     <button disabled={busy||!question.trim()}>{busy?'MAGNANIMOUS IS WORKING…':'ASK MAGNANIMOUS'}</button>
    </form>
    <div className={styles.answer}>{answer||'Magnanimous AI remains the reasoning and command layer. Customers subscribe to Magnanimous Telecom service; outside carriers, MVNOs, telecom cores, charging systems and vendors remain replaceable infrastructure tools beneath it.'}</div>
   </div>
  </section>
