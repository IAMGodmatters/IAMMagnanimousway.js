...
const products=[
 {name:'Christian Scripture Ring',image:'https://file.zendrop.com/products/5b/69/dc6c0d8f4cb3b10ea1f676816308_import.webp',type:'Faith Jewelry',price:'₱899'},
 {name:'Christian Cross Necklace',image:'https://file.zendrop.com/products/c2/55/946375624119950d1496f601387d_import.webp',type:'Faith Jewelry',price:'₱999'},
 {name:'Prayer Hands Pendant Necklace',image:'https://file.zendrop.com/products/06/72/3f1830314fc686e6a8f4d8ed7f9b_import.webp',type:'Faith Jewelry',price:'₱1,199'}
];
...
export default function Shop(){
 return <main className="shop">
  ...
  <section id="featured" className="featured">
   ...
   <div className="grid">{products.map(p=><article key={p.name}><div className="image"><img src={p.image} alt={p.name}/></div><small>{p.type}</small><h3>{p.name}</h3><div className="price">{p.price}</div><a href={STORE_URL} target="_blank" rel="noopener noreferrer">Shop This Item →</a></article>)}</div>
  </section>

  <section className="upsell">
   <div className="upsellCopy"><small>COMPLETE THE FAITH GIFT</small><h2>Pair two meaningful pieces together.</h2><p>Buying a ring? Add a cross necklace. Choosing a necklace? Add the Prayer Hands pendant as a second faith-centered gift. Mix and match from the featured collection before checkout.</p></div>
   <div className="bundleGrid"><div><b>Ring + Cross</b><span>Two everyday faith pieces</span><strong>₱1,898</strong></div><div><b>Cross + Prayer Hands</b><span>A meaningful gift pairing</span><strong>₱2,198</strong></div><div><b>Complete 3-Piece Set</b><span>All featured jewelry together</span><strong>₱3,097</strong></div></div>
   <a className="bundleCta" href={STORE_URL} target="_blank" rel="noopener noreferrer">Build Your Faith Gift Set →</a>
  </section>
