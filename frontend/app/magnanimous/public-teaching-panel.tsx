...
export default function PublicTeachingPanel(){
 ...
 return <section className="teach" aria-labelledby="teach-magnanimous-title">
  <div className="head"><div><small>PUBLIC QA + TEACHING</small><h2 id="teach-magnanimous-title">Teach Magnanimous AI™</h2><p>Anyone can contribute a better method, example, source, or difficult QA challenge. Every submission enters an automatic quality review. Teaching that clears the quality and safety gate is applied to the selected specialist automatically; anything uncertain is held for owner oversight.</p></div><span className="badge">AUTOMATIC QA REVIEW</span></div>
  ...
  <label>Proposed lesson / better method<textarea value={content} onChange={e=>setContent(e.target.value)} placeholder="Explain the procedure, expert method, correction, example, or stronger answer pattern you want Magnanimous to review…"/></label>
  ...
  <div className="foot"><p>No account is required. Common personal identifiers and credentials are not needed; do not submit passwords, API keys, payment details, or private records.</p><button type="button" disabled={busy||!selected} onClick={submit}>{busy?'Submitting…':'Submit Teaching for QA Review →'}</button></div>
  <style jsx>{`
