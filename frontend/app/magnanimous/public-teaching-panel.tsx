...
export default function PublicTeachingPanel(){
 ...
 async function submit(){
  ...
  if(!content.trim()&&!challenge.trim()&&!url.trim()){setErr('Add teaching, a correction, a QA challenge, or an optional supporting source before submitting.');return}
  ...
  try{
   ...
   setTitle('');setTags('');setContent('');setUrl('');setChallenge('');setExpected('');setWebsite('');setMsg(d.message||'Thank you. Your teaching entered automatic QA review. Material that passes the quality gate is learned automatically; anything held remains available for owner review.');
 ...
 return <section className="teach" aria-labelledby="teach-magnanimous-title">
  <div className="head"><div><small>PUBLIC QA + TEACHING</small><h2 id="teach-magnanimous-title">Teach Magnanimous AI™</h2><p>Anyone can contribute a better method, example, correction, source, or difficult QA challenge. Only the specialist branch and useful teaching/QA content are needed. A title, skill tags, and public source URL are optional. Every submission enters automatic quality review; uncertain material is held for owner oversight.</p></div><span className="badge">AUTOMATIC QA REVIEW</span></div>
  ...
  <div className="grid">
   ...
   <label>Title <span className="optional">(optional)</span><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Optional short title"/></label>
   <label>Skill tags <span className="optional">(optional)</span><input value={tags} onChange={e=>setTags(e.target.value)} placeholder="Optional: research, sales, Bible study, support…"/></label>
   <label>Public source URL <span className="optional">(optional)</span><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="Optional supporting link" inputMode="url"/></label>
  </div>
  {agent&&<p className="agent"><b>{agent.name}</b> — {agent.description}</p>}
  <label>Teaching, correction, or better method <span className="optional">(optional if submitting only a QA challenge)</span><textarea value={content} onChange={e=>setContent(e.target.value)} placeholder="Explain what Magnanimous should learn, correct, improve, or do better. You may leave this blank if you are submitting only a QA challenge…"/></label>
  <div className="two"><label>QA challenge <span className="optional">(optional)</span><textarea value={challenge} onChange={e=>setChallenge(e.target.value)} placeholder="Give Magnanimous or the specialist a difficult question, edge case, failure mode, or real-world scenario…"/></label><label>Expected strong outcome <span className="optional">(needed only when you submit a QA challenge)</span><textarea value={expected} onChange={e=>setExpected(e.target.value)} placeholder="For a QA challenge, describe what a strong answer should contain or accomplish…"/></label></div>
