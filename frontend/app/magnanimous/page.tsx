...
type ChatMessage={id:number;role:Role;content:string;meta?:string;sources?:Array<{title?:string;url?:string}>;assistantName?:string;assistantTitle?:string;silentVoice?:boolean};
...
export default function StandaloneMagnanimous(){
 ...
 const[lastFailed,setLastFailed]=useState<{text:string;mode:string}|null>(null);
 ...
 async function send(e?:FormEvent,retry?:{text:string;mode:string}){
  ...
  if(!text||busy)return;
  const requestMode=MODES.find(x=>x.id===(retry?.mode??mode))||activeMode;
  const userId=nextId;
  setNextId(v=>v+2);
  setMessages(v=>[...v,{id:userId,role:'user',content:text}]);
  if(!retry)setInput('');
  setLastFailed(null);setBusy(true);
  try{
   ...
   setMessages(v=>[...v,{id:userId+1,role:'assistant',assistantName:'Magnanimous AI',silentVoice:true,content:`Connection interrupted: ${String(err?.message||err)}. Your request is preserved. Use Retry to run it again without retyping it.`}]);
  ...
  <section className="mag-shell">
   ...
   <section className="mag-chat" aria-label="Magnanimous conversation">
    ...
    <form className="mag-compose" onSubmit={send}>
     ...
     <div className="mag-compose-row"><span>Magnanimous answers directly and automatically hands specialty questions to the matching named AI. Shift+Enter for a new line.</span><div>{lastFailed&&<button type="button" className="retry" disabled={busy} onClick={()=>send(undefined,lastFailed)}>{busy?'Working…':'Retry ↻'}</button>}<button type="button" className="secondary" onClick={copyLast}>Copy last</button><button type="submit" disabled={busy||!input.trim()}>{busy?'Working…':'Send →'}</button></div></div>
    </form>
   </section>
  </section>
