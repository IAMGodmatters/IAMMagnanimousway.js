...
type ChatMessage={id:number;role:Role;content:string;meta?:string;sources?:Array<{title?:string;url?:string}>};
...
export default function StandaloneMagnanimous(){
 ...
 function clearChat(){
  setMessages([{id:Date.now(),role:'assistant',content:'New conversation started. The Magnanimous brain itself has not been reset; signed-in memory and learned knowledge remain available.'}]);
 }
 ...
 return <main className="mag-standalone" id="iam-main">
  ...
  <section className="mag-shell">
   <aside className="mag-rail" aria-label="Magnanimous modes">
    <div className="mag-core"><span>M</span><b>ONE BRAIN</b><small>Many capabilities</small></div>
    <nav>{MODES.map(x=><button key={x.id} className={mode===x.id?'active':''} onClick={()=>setMode(x.id)}><b>{x.label}</b><span>{x.hint}</span></button>)}</nav>
    <div className="mag-memory"><b>{signedIn?'Persistent learning on':'Guest session'}</b><span>{signedIn?'Your authorized memory and learned workspace knowledge can be reused here.':'Direct AI access is available. Sign in when you want persistent memory or connected-account actions.'}</span>{!signedIn&&<a href="/login?returnTo=%2Fmagnanimous">Sign in for memory →</a>}</div>
   </aside>

   <section className="mag-chat" aria-label="Magnanimous conversation">
    <div className="mag-modebar"><div><small>ACTIVE MODE</small><b>{activeMode.label}</b></div><span>Same central Magnanimous brain • no duplicate AI</span></div>
    <div className="mag-thread" aria-live="polite">
     {messages.map(m=><article key={m.id} className={`mag-message ${m.role}`}>
      <div className="mag-avatar">{m.role==='assistant'?'M':'YOU'}</div>
      <div className="mag-bubble"><small>{m.role==='assistant'?'MAGNANIMOUS AI':'YOU'}</small><p>{m.content}</p>{m.meta&&<div className="mag-meta">{m.meta}</div>}{m.sources&&m.sources.length>0&&<div className="mag-sources"><b>Sources</b>{m.sources.map((s,i)=>s.url?<a key={`${s.url}-${i}`} href={s.url} target="_blank" rel="noreferrer">{s.title||s.url}</a>:<span key={i}>{s.title}</span>)}</div>}</div>
     </article>)}
     {busy&&<article className="mag-message assistant"><div className="mag-avatar">M</div><div className="mag-bubble"><small>MAGNANIMOUS AI</small><div className="mag-thinking"><i/><i/><i/><span>Thinking, routing and checking…</span></div></div></article>}
     <div ref={endRef}/>
    </div>
    <form className="mag-compose" onSubmit={send}>
     ...
     <div className="mag-compose-row"><span>Readable public links can be absorbed into Magnanimous knowledge. Shift+Enter for a new line.</span><div><button type="button" className="secondary" onClick={copyLast}>Copy last</button><button type="submit" disabled={busy||!input.trim()}>{busy?'Working…':'Send →'}</button></div></div>
    </form>
   </section>
  </section>

  <footer className="mag-foot"><span>MAGNANIMOUS AI™</span><b>Central brain • independent interface</b><span>Knowledge and actions remain permission-aware.</span></footer>
