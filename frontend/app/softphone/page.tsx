...
export default function Softphone(){
 ...
 async function startNative(cfg:Config,active:string){
  ...
  if(session.pstn_direct!==false||!session.session_id||!session.username||!session.password||!session.domain||!session.wss_url||!session.allowed_call_scope?.includes('internal-magnanimous'))throw new Error('Native PBX returned an invalid or over-broad browser session.');
  nativeSessionRef.current=session.session_id;
  let phone:MagnanimousNativePhone;
  try{
   phone=new MagnanimousNativePhone(session,remoteAudioRef.current,{
   ...
   await phone.start();
  }catch(e){await cleanupNative(active,false);throw e}
 ...
 async function createDevice(cfg:Config,active:string){
  ...
  device.on('registered',async()=>{setCompatOnline(true);setStatus(cfg.native_pbx_live&&cfg.native_session_ready?'Native PBX + guarded PSTN fallback ready':`Compatibility PSTN ready as ${cfg.identity||'agent'}`);await presence(active,'available').catch(()=>{})});
