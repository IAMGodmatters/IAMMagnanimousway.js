...
export default function RecoveryContacts(){
 ...
 async function load(){
  ...
  try{
   ...
   if(r.ok){setEmailHint(String(d.recovery_email||''));setEmailVerified(Boolean(d.recovery_email_verified));setPhoneHint(String(d.phone||''))}
  }catch{}
 }
 useEffect(()=>{void load()},[]);
 ...
 return <section className="contacts">
  ...
  <div className="contactGrid">
   <div className="contactCard">
    ...
    {!emailChallenge&&<div className="row"><input type="email" autoComplete="email" placeholder="alternate@example.com" value={recoveryEmail} onChange={e=>setRecoveryEmail(e.target.value)}/><button disabled={busy||!recoveryEmail} onClick={requestEmail}>SEND 8-DIGIT CODE</button></div>}
