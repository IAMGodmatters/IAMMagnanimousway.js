...
export default function OwnerLoginPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    } catch (err: any) { setError(err?.message || 'Unable to sign in as owner.'); }
  ...
  return (
    <main style={styles.page}>
      ...
      <section style={styles.card}>
        ...
        <p style={styles.sub}>Sign in with your existing owner credentials to manage the platform, revenue links, and private CRM.</p>
        <form onSubmit={submit} style={styles.form}>
          ...
          <button disabled={busy} type="submit" style={styles.button}>{busy ? 'Signing in…' : 'Sign In as Owner'}</button>
          {error && <div role="alert" style={styles.error}>{error}</div>}
        </form>
        <p style={styles.switch}>Need a regular account? <a href="/signup" style={styles.link}>Create an account</a></p>
        <p style={styles.switch}>Already have a regular account? <a href="/login" style={styles.link}>Sign in</a></p>
