...
export default function LoginPage() {
  ...
  async function finishSession(d:any){
    if(!d?.token)throw new Error(d?.detail||'Secure sign-in could not be completed.');
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if(mfaToken){
        const r=await fetch(`${api}/api/auth/totp/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mfa_token:mfaToken,code:mfaCode})});
        const d=await readResponse(r);
        if(!r.ok||!d.token)throw new Error(d.detail||'Authenticator verification failed.');
        await finishSession(d);
        return;
      }
      const r = await fetch(`${api}/api/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
      const d = await readResponse(r);
      if(!r.ok)throw new Error(d.detail||'Invalid email or password.');
      if(d.mfa_required&&d.mfa_token){
        setMfaToken(String(d.mfa_token));setMfaHint(String(d.email_hint||''));setMfaCode('');
        return;
      }
      await finishSession(d);
  ...
  return <main className="portal">
    ...
    <a className="publicHome" href="/solutions">← About the platform</a>
    <button className="sound" aria-pressed={soundOn} onClick={toggleSound}>{soundOn ? '🔊 SOUND: ON' : '🔇 SOUND: OFF'}</button>
    ...
    <section className="card">
      ...
      <div className="badge">CUSTOMER WORKSPACE ACCESS</div>
      <h1>WELCOME BACK TO YOUR WORKSPACE</h1>
      <div className="lock">▣ ENCRYPTED WORKSPACE CONNECTION</div>
      <p>This is the regular customer sign-in for your private AI, business, calling, CRM, content and creator workspace.</p>
      <form onSubmit={submit}>
        ...
        <button className="enter" disabled={busy} type="submit">{busy?'VERIFYING SECURE ACCESS…':mfaToken?'VERIFY AUTHENTICATOR CODE  ›':'ENTER MY WORKSPACE  ›'}</button>
      ...
      <div className="entryChoices">
        ...
        <a className="owner" href="/owner-login"><b>PLATFORM OWNER ONLY</b><span>Open owner sign-in →</span></a>
      </div>
    </section>
