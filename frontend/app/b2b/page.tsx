...
export default function B2BPage(){
 ...
 return <main className="page">
  ...
  <section className="stats">
   ...
   <article><small>TRAVEL CONTRACTS</small><b>{data?.summary?.travel_contracts||0}</b></article>
   <article><small>CONNECTION TEMPLATES</small><b>{data?.summary?.connection_templates||0}</b></article>
   <article><small>LIVE CREDENTIAL SETS</small><b>{(data?.connections||[]).filter((x:any)=>x.configured).length}</b></article>
  </section>
  <section className="grid">
   <article className="panel"><small>MAGNANIMOUS B2B PLANNER</small><h2>Turn an opportunity into a connection plan</h2><textarea value={goal} onChange={e=>setGoal(e.target.value)} /><button onClick={build} disabled={busy||!goal.trim()}>{busy?'Building…':'Build B2B plan'}</button>{plan&&<div className="plan"><b>{plan.families?.join(' + ')}</b>{(plan.sequence||[]).map((x:string,i:number)=><p key={i}><span>{i+1}</span>{x}</p>)}</div>}</article>
   <article className="panel"><small>CORE RULE</small><h2>Magnanimous owns the business workflow</h2><p>Supplier, airline, GDS, hotel, tour, ERP, payment and marketplace accounts provide authorized inventory or execution only. They do not own Magnanimous memory, policy, CRM, pricing logic, audit trails or learning.</p><div className="chips">{['Wholesale','RFQ / Quotes','Purchase Orders','Private Label','Dropshipping','Inventory','ERP / EDI','Flights','GDS / NDC','Hotels','Tours','Sub-agents','Corporate Travel','Markups','Servicing'].map(x=><span key={x}>{x}</span>)}</div></article>
  </section>
  <section className="panel connections"><div className="head"><div><small>CONNECTION FABRIC</small><h2>Provider and partner readiness</h2></div><select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All connection families</option>{families.map((x:any)=><option key={x} value={x}>{x}</option>)}</select></div>
   <p className="muted">“Configured” means credentials exist. It does not mean commercial approval, ticketing authority, merchant status, accreditation, or a successful live transaction has been verified.</p>
   <div className="cards">{connections.map((c:any)=><article key={c.id} className={c.configured?'ready':''}><div><b>{c.name}</b><span>{c.family} · {c.mode}</span></div><em>{c.configured?'CREDENTIALS PRESENT':c.status.replaceAll('-',' ').toUpperCase()}</em><p>{c.note}</p></article>)}</div>
  </section>
